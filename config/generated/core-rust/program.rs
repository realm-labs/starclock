...
impl super::runtime::SoraDecode for Program {
    fn decode(
...
pub struct ProgramTable {
    keys: Vec<i32>,
    rows: SoraMap<i32, Program>,
}

impl ProgramTable {
    pub const NAME: &'static str = "Program";
    ...
    pub(super) fn from_rows(rows: Vec<Program>) -> Result<Self, super::runtime::SoraReadError> {
    ...
    pub fn get(&self, key: &i32) -> Option<&Program> {
        self.rows.get(key)
    }
    ...
    pub fn ordered_rows(&self) -> impl Iterator<Item = &Program> {
        self.keys.iter().filter_map(|key| self.rows.get(key))
    }
}

impl std::ops::Deref for ProgramTable {
    type Target = SoraMap<i32, Program>;
...
impl super::ErasedSoraTable for ProgramTable {
...
impl super::SoraKeyedTable for ProgramTable {
    type Key = i32;
    type Row = Program;
