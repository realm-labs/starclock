...
pub struct CurrencyWarsDifficulties {
    #[serde(rename = "id")]
    pub id: i32,
...
#[derive(Debug, Clone)]
pub struct CurrencyWarsDifficultiesTable {
    keys: Vec<i32>,
    rows: SoraMap<i32, CurrencyWarsDifficulties>,
    by_stable_key: SoraMap<String, i32>,
}

impl CurrencyWarsDifficultiesTable {
    pub const NAME: &'static str = "CurrencyWarsDifficulties";
    pub const INFO: super::SoraTableInfo = super::SoraTableInfo {
        name: Self::NAME,
        row_type: "CurrencyWarsDifficulties",
    ...
    pub(super) fn from_rows(rows: Vec<CurrencyWarsDifficulties>) -> Result<Self, super::runtime::SoraReadError> {
    ...
    pub fn get(&self, key: &i32) -> Option<&CurrencyWarsDifficulties> {
        self.rows.get(key)
    }
    ...
    pub fn ordered_rows(&self) -> impl Iterator<Item = &CurrencyWarsDifficulties> {
        self.keys.iter().filter_map(|key| self.rows.get(key))
    }
    pub fn get_by_stable_key(&self, stable_key: &str) -> Option<&CurrencyWarsDifficulties> {
        self.by_stable_key.get(stable_key).and_then(|key| self.rows.get(key))
    }
}

impl std::ops::Deref for CurrencyWarsDifficultiesTable {
    type Target = SoraMap<i32, CurrencyWarsDifficulties>;
...
impl super::ErasedSoraTable for CurrencyWarsDifficultiesTable {
...
impl super::SoraKeyedTable for CurrencyWarsDifficultiesTable {
    type Key = i32;
    type Row = CurrencyWarsDifficulties;
