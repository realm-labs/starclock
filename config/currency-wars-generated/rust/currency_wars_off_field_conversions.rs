...
pub struct CurrencyWarsOffFieldConversions {
    #[serde(rename = "id")]
    pub id: i32,
...
impl CurrencyWarsOffFieldConversions {
...
#[derive(Debug, Clone)]
pub struct CurrencyWarsOffFieldConversionsTable {
    keys: Vec<i32>,
    rows: SoraMap<i32, CurrencyWarsOffFieldConversions>,
    by_stable_key: SoraMap<String, i32>,
}

impl CurrencyWarsOffFieldConversionsTable {
    pub const NAME: &'static str = "CurrencyWarsOffFieldConversions";
    pub const INFO: super::SoraTableInfo = super::SoraTableInfo {
        name: Self::NAME,
        row_type: "CurrencyWarsOffFieldConversions",
    ...
    pub(super) fn from_rows(rows: Vec<CurrencyWarsOffFieldConversions>) -> Result<Self, super::runtime::SoraReadError> {
    ...
    pub fn get(&self, key: &i32) -> Option<&CurrencyWarsOffFieldConversions> {
        self.rows.get(key)
    }
    ...
    pub fn ordered_rows(&self) -> impl Iterator<Item = &CurrencyWarsOffFieldConversions> {
        self.keys.iter().filter_map(|key| self.rows.get(key))
    }
    pub fn get_by_stable_key(&self, stable_key: &str) -> Option<&CurrencyWarsOffFieldConversions> {
        self.by_stable_key.get(stable_key).and_then(|key| self.rows.get(key))
    }
}

impl std::ops::Deref for CurrencyWarsOffFieldConversionsTable {
    type Target = SoraMap<i32, CurrencyWarsOffFieldConversions>;
...
impl super::ErasedSoraTable for CurrencyWarsOffFieldConversionsTable {
...
impl super::SoraKeyedTable for CurrencyWarsOffFieldConversionsTable {
    type Key = i32;
    type Row = CurrencyWarsOffFieldConversions;
