...
pub struct CurrencyWarsPackIndex {
    #[serde(rename = "id")]
    pub id: i32,
...
#[derive(Debug, Clone)]
pub struct CurrencyWarsPackIndexTable {
    keys: Vec<i32>,
    rows: SoraMap<i32, CurrencyWarsPackIndex>,
    by_stable_key: SoraMap<String, i32>,
}

impl CurrencyWarsPackIndexTable {
    pub const NAME: &'static str = "CurrencyWarsPackIndex";
    pub const INFO: super::SoraTableInfo = super::SoraTableInfo {
        name: Self::NAME,
        row_type: "CurrencyWarsPackIndex",
    ...
    pub(super) fn from_rows(rows: Vec<CurrencyWarsPackIndex>) -> Result<Self, super::runtime::SoraReadError> {
    ...
    pub fn get(&self, key: &i32) -> Option<&CurrencyWarsPackIndex> {
        self.rows.get(key)
    }
    ...
    pub fn ordered_rows(&self) -> impl Iterator<Item = &CurrencyWarsPackIndex> {
        self.keys.iter().filter_map(|key| self.rows.get(key))
    }
    pub fn get_by_stable_key(&self, stable_key: &str) -> Option<&CurrencyWarsPackIndex> {
        self.by_stable_key.get(stable_key).and_then(|key| self.rows.get(key))
    }
}

impl std::ops::Deref for CurrencyWarsPackIndexTable {
    type Target = SoraMap<i32, CurrencyWarsPackIndex>;
...
impl super::ErasedSoraTable for CurrencyWarsPackIndexTable {
...
impl super::SoraKeyedTable for CurrencyWarsPackIndexTable {
    type Key = i32;
    type Row = CurrencyWarsPackIndex;
