...
pub struct CurrencyWarsMechanicSourceFiles {
    #[serde(rename = "id")]
    pub id: i32,
...
impl CurrencyWarsMechanicSourceFiles {
...
#[derive(Debug, Clone)]
pub struct CurrencyWarsMechanicSourceFilesTable {
    keys: Vec<i32>,
    rows: SoraMap<i32, CurrencyWarsMechanicSourceFiles>,
    by_stable_key: SoraMap<String, i32>,
}

impl CurrencyWarsMechanicSourceFilesTable {
    pub const NAME: &'static str = "CurrencyWarsMechanicSourceFiles";
    pub const INFO: super::SoraTableInfo = super::SoraTableInfo {
        name: Self::NAME,
        row_type: "CurrencyWarsMechanicSourceFiles",
    ...
    pub(super) fn from_rows(rows: Vec<CurrencyWarsMechanicSourceFiles>) -> Result<Self, super::runtime::SoraReadError> {
    ...
    pub fn get(&self, key: &i32) -> Option<&CurrencyWarsMechanicSourceFiles> {
        self.rows.get(key)
    }
    ...
    pub fn ordered_rows(&self) -> impl Iterator<Item = &CurrencyWarsMechanicSourceFiles> {
        self.keys.iter().filter_map(|key| self.rows.get(key))
    }
    pub fn get_by_stable_key(&self, stable_key: &str) -> Option<&CurrencyWarsMechanicSourceFiles> {
        self.by_stable_key.get(stable_key).and_then(|key| self.rows.get(key))
    }
}

impl std::ops::Deref for CurrencyWarsMechanicSourceFilesTable {
    type Target = SoraMap<i32, CurrencyWarsMechanicSourceFiles>;
...
impl super::ErasedSoraTable for CurrencyWarsMechanicSourceFilesTable {
...
impl super::SoraKeyedTable for CurrencyWarsMechanicSourceFilesTable {
    type Key = i32;
    type Row = CurrencyWarsMechanicSourceFiles;
