...
pub struct CurrencyWarsBossPools {
    #[serde(rename = "id")]
    pub id: i32,
...
#[derive(Debug, Clone)]
pub struct CurrencyWarsBossPoolsTable {
    keys: Vec<i32>,
    rows: SoraMap<i32, CurrencyWarsBossPools>,
    by_stable_key: SoraMap<String, i32>,
}

impl CurrencyWarsBossPoolsTable {
    pub const NAME: &'static str = "CurrencyWarsBossPools";
    pub const INFO: super::SoraTableInfo = super::SoraTableInfo {
        name: Self::NAME,
        row_type: "CurrencyWarsBossPools",
    ...
    pub(super) fn from_rows(rows: Vec<CurrencyWarsBossPools>) -> Result<Self, super::runtime::SoraReadError> {
    ...
    pub fn get(&self, key: &i32) -> Option<&CurrencyWarsBossPools> {
        self.rows.get(key)
    }
    ...
    pub fn ordered_rows(&self) -> impl Iterator<Item = &CurrencyWarsBossPools> {
        self.keys.iter().filter_map(|key| self.rows.get(key))
    }
    pub fn get_by_stable_key(&self, stable_key: &str) -> Option<&CurrencyWarsBossPools> {
        self.by_stable_key.get(stable_key).and_then(|key| self.rows.get(key))
    }
}

impl std::ops::Deref for CurrencyWarsBossPoolsTable {
    type Target = SoraMap<i32, CurrencyWarsBossPools>;
...
impl super::ErasedSoraTable for CurrencyWarsBossPoolsTable {
...
impl super::SoraKeyedTable for CurrencyWarsBossPoolsTable {
    type Key = i32;
    type Row = CurrencyWarsBossPools;
