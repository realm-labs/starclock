...
pub struct CurrencyWarsFormulaRecipes {
    #[serde(rename = "id")]
    pub id: i32,
...
impl CurrencyWarsFormulaRecipes {
...
#[derive(Debug, Clone)]
pub struct CurrencyWarsFormulaRecipesTable {
    keys: Vec<i32>,
    rows: SoraMap<i32, CurrencyWarsFormulaRecipes>,
    by_stable_key: SoraMap<String, i32>,
}

impl CurrencyWarsFormulaRecipesTable {
    pub const NAME: &'static str = "CurrencyWarsFormulaRecipes";
    pub const INFO: super::SoraTableInfo = super::SoraTableInfo {
        name: Self::NAME,
        row_type: "CurrencyWarsFormulaRecipes",
    ...
    pub(super) fn from_rows(rows: Vec<CurrencyWarsFormulaRecipes>) -> Result<Self, super::runtime::SoraReadError> {
    ...
    pub fn get(&self, key: &i32) -> Option<&CurrencyWarsFormulaRecipes> {
        self.rows.get(key)
    }
    ...
    pub fn ordered_rows(&self) -> impl Iterator<Item = &CurrencyWarsFormulaRecipes> {
        self.keys.iter().filter_map(|key| self.rows.get(key))
    }
    pub fn get_by_stable_key(&self, stable_key: &str) -> Option<&CurrencyWarsFormulaRecipes> {
        self.by_stable_key.get(stable_key).and_then(|key| self.rows.get(key))
    }
}

impl std::ops::Deref for CurrencyWarsFormulaRecipesTable {
    type Target = SoraMap<i32, CurrencyWarsFormulaRecipes>;
...
impl super::ErasedSoraTable for CurrencyWarsFormulaRecipesTable {
...
impl super::SoraKeyedTable for CurrencyWarsFormulaRecipesTable {
    type Key = i32;
    type Row = CurrencyWarsFormulaRecipes;
