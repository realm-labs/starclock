...
pub struct CurrencyWarsEnemySlots {
    #[serde(rename = "id")]
    pub id: i32,
...
#[derive(Debug, Clone)]
pub struct CurrencyWarsEnemySlotsTable {
    keys: Vec<i32>,
    rows: SoraMap<i32, CurrencyWarsEnemySlots>,
    by_stable_key: SoraMap<String, i32>,
}

impl CurrencyWarsEnemySlotsTable {
    pub const NAME: &'static str = "CurrencyWarsEnemySlots";
    pub const INFO: super::SoraTableInfo = super::SoraTableInfo {
        name: Self::NAME,
        row_type: "CurrencyWarsEnemySlots",
    ...
    pub(super) fn from_rows(rows: Vec<CurrencyWarsEnemySlots>) -> Result<Self, super::runtime::SoraReadError> {
    ...
    pub fn get(&self, key: &i32) -> Option<&CurrencyWarsEnemySlots> {
        self.rows.get(key)
    }
    ...
    pub fn ordered_rows(&self) -> impl Iterator<Item = &CurrencyWarsEnemySlots> {
        self.keys.iter().filter_map(|key| self.rows.get(key))
    }
    pub fn get_by_stable_key(&self, stable_key: &str) -> Option<&CurrencyWarsEnemySlots> {
        self.by_stable_key.get(stable_key).and_then(|key| self.rows.get(key))
    }
}

impl std::ops::Deref for CurrencyWarsEnemySlotsTable {
    type Target = SoraMap<i32, CurrencyWarsEnemySlots>;
...
impl super::ErasedSoraTable for CurrencyWarsEnemySlotsTable {
...
impl super::SoraKeyedTable for CurrencyWarsEnemySlotsTable {
    type Key = i32;
    type Row = CurrencyWarsEnemySlots;
