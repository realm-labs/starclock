...
pub struct CurrencyWarsBlessingPaths {
    #[serde(rename = "id")]
    pub id: i32,
...
impl CurrencyWarsBlessingPaths {
...
#[derive(Debug, Clone)]
pub struct CurrencyWarsBlessingPathsTable {
    keys: Vec<i32>,
    rows: SoraMap<i32, CurrencyWarsBlessingPaths>,
    by_stable_key: SoraMap<String, i32>,
}

impl CurrencyWarsBlessingPathsTable {
    pub const NAME: &'static str = "CurrencyWarsBlessingPaths";
    pub const INFO: super::SoraTableInfo = super::SoraTableInfo {
        name: Self::NAME,
        row_type: "CurrencyWarsBlessingPaths",
    ...
    pub(super) fn from_rows(rows: Vec<CurrencyWarsBlessingPaths>) -> Result<Self, super::runtime::SoraReadError> {
    ...
    pub fn get(&self, key: &i32) -> Option<&CurrencyWarsBlessingPaths> {
        self.rows.get(key)
    }
    ...
    pub fn ordered_rows(&self) -> impl Iterator<Item = &CurrencyWarsBlessingPaths> {
        self.keys.iter().filter_map(|key| self.rows.get(key))
    }
    pub fn get_by_stable_key(&self, stable_key: &str) -> Option<&CurrencyWarsBlessingPaths> {
        self.by_stable_key.get(stable_key).and_then(|key| self.rows.get(key))
    }
}

impl std::ops::Deref for CurrencyWarsBlessingPathsTable {
    type Target = SoraMap<i32, CurrencyWarsBlessingPaths>;
...
impl super::ErasedSoraTable for CurrencyWarsBlessingPathsTable {
...
impl super::SoraKeyedTable for CurrencyWarsBlessingPathsTable {
    type Key = i32;
    type Row = CurrencyWarsBlessingPaths;
