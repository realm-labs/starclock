...
pub struct CurrencyWarsBlessings {
    #[serde(rename = "id")]
    pub id: i32,
...
#[derive(Debug, Clone)]
pub struct CurrencyWarsBlessingsTable {
    keys: Vec<i32>,
    rows: SoraMap<i32, CurrencyWarsBlessings>,
    by_stable_key: SoraMap<String, i32>,
}

impl CurrencyWarsBlessingsTable {
    pub const NAME: &'static str = "CurrencyWarsBlessings";
    pub const INFO: super::SoraTableInfo = super::SoraTableInfo {
        name: Self::NAME,
        row_type: "CurrencyWarsBlessings",
    ...
    pub(super) fn from_rows(rows: Vec<CurrencyWarsBlessings>) -> Result<Self, super::runtime::SoraReadError> {
    ...
    pub fn get(&self, key: &i32) -> Option<&CurrencyWarsBlessings> {
        self.rows.get(key)
    }
    ...
    pub fn ordered_rows(&self) -> impl Iterator<Item = &CurrencyWarsBlessings> {
        self.keys.iter().filter_map(|key| self.rows.get(key))
    }
    pub fn get_by_stable_key(&self, stable_key: &str) -> Option<&CurrencyWarsBlessings> {
        self.by_stable_key.get(stable_key).and_then(|key| self.rows.get(key))
    }
}

impl std::ops::Deref for CurrencyWarsBlessingsTable {
    type Target = SoraMap<i32, CurrencyWarsBlessings>;
...
impl super::ErasedSoraTable for CurrencyWarsBlessingsTable {
...
impl super::SoraKeyedTable for CurrencyWarsBlessingsTable {
    type Key = i32;
    type Row = CurrencyWarsBlessings;
