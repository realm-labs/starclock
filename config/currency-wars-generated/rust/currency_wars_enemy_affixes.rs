...
pub struct CurrencyWarsEnemyAffixes {
    #[serde(rename = "id")]
    pub id: i32,
...
#[derive(Debug, Clone)]
pub struct CurrencyWarsEnemyAffixesTable {
    keys: Vec<i32>,
    rows: SoraMap<i32, CurrencyWarsEnemyAffixes>,
    by_stable_key: SoraMap<String, i32>,
}

impl CurrencyWarsEnemyAffixesTable {
    pub const NAME: &'static str = "CurrencyWarsEnemyAffixes";
    pub const INFO: super::SoraTableInfo = super::SoraTableInfo {
        name: Self::NAME,
        row_type: "CurrencyWarsEnemyAffixes",
    ...
    pub(super) fn from_rows(rows: Vec<CurrencyWarsEnemyAffixes>) -> Result<Self, super::runtime::SoraReadError> {
    ...
    pub fn get(&self, key: &i32) -> Option<&CurrencyWarsEnemyAffixes> {
        self.rows.get(key)
    }
    ...
    pub fn ordered_rows(&self) -> impl Iterator<Item = &CurrencyWarsEnemyAffixes> {
        self.keys.iter().filter_map(|key| self.rows.get(key))
    }
    pub fn get_by_stable_key(&self, stable_key: &str) -> Option<&CurrencyWarsEnemyAffixes> {
        self.by_stable_key.get(stable_key).and_then(|key| self.rows.get(key))
    }
}

impl std::ops::Deref for CurrencyWarsEnemyAffixesTable {
    type Target = SoraMap<i32, CurrencyWarsEnemyAffixes>;
...
impl super::ErasedSoraTable for CurrencyWarsEnemyAffixesTable {
...
impl super::SoraKeyedTable for CurrencyWarsEnemyAffixesTable {
    type Key = i32;
    type Row = CurrencyWarsEnemyAffixes;
