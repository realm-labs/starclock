...
pub struct CurrencyWarsBuildMappings {
    #[serde(rename = "id")]
    pub id: i32,
...
impl CurrencyWarsBuildMappings {
...
#[derive(Debug, Clone)]
pub struct CurrencyWarsBuildMappingsTable {
    keys: Vec<i32>,
    rows: SoraMap<i32, CurrencyWarsBuildMappings>,
    by_stable_key: SoraMap<String, i32>,
}

impl CurrencyWarsBuildMappingsTable {
    pub const NAME: &'static str = "CurrencyWarsBuildMappings";
    pub const INFO: super::SoraTableInfo = super::SoraTableInfo {
        name: Self::NAME,
        row_type: "CurrencyWarsBuildMappings",
    ...
    pub(super) fn from_rows(rows: Vec<CurrencyWarsBuildMappings>) -> Result<Self, super::runtime::SoraReadError> {
    ...
    pub fn get(&self, key: &i32) -> Option<&CurrencyWarsBuildMappings> {
        self.rows.get(key)
    }
    ...
    pub fn ordered_rows(&self) -> impl Iterator<Item = &CurrencyWarsBuildMappings> {
        self.keys.iter().filter_map(|key| self.rows.get(key))
    }
    pub fn get_by_stable_key(&self, stable_key: &str) -> Option<&CurrencyWarsBuildMappings> {
        self.by_stable_key.get(stable_key).and_then(|key| self.rows.get(key))
    }
}

impl std::ops::Deref for CurrencyWarsBuildMappingsTable {
    type Target = SoraMap<i32, CurrencyWarsBuildMappings>;
...
impl super::ErasedSoraTable for CurrencyWarsBuildMappingsTable {
...
impl super::SoraKeyedTable for CurrencyWarsBuildMappingsTable {
    type Key = i32;
    type Row = CurrencyWarsBuildMappings;
