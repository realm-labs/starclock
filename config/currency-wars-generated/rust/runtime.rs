...
const SORA_BUNDLE_VERSION: u32 = 2;
...
impl LocalePack {
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, SoraReadError> {
        ...
        if version != 1 {
            return Err(SoraReadError::new(format!("unsupported Sora i18n pack version {}", version)));
        }
        ...
        for _ in 0..count {
            ...
            if value.is_empty() {
                return Err(SoraReadError::new(format!("text key `{key}` has empty text")));
            }
    ...
    pub fn get(&self, key: &TextKey) -> Option<&str> {
        self.translations.get(key.as_str()).map(|value| value.as_ref())
    }
}
...
impl<'a> SoraBundle<'a> {
    pub fn parse(bytes: &'a [u8]) -> Result<Self, SoraReadError> {
        ...
        for _ in 0..section_count {
            ...
            match kind {
                ...
                SECTION_KIND_STRINGS => {
                    strings_count += 1;
                    if name != "$strings" {
                        return Err(SoraReadError::new("Sora string table section must be named `$strings`"));
                    }
                }
                _ => {
        ...
        if strings_count != 1 {
            return Err(SoraReadError::new(format!("expected exactly 1 Sora string table section, got {}", strings_count)));
        }
...
fn decode_rows<T: SoraDecode>(payload: &[u8], strings: &[std::sync::Arc<str>]) -> Result<Vec<T>, SoraReadError> {
    if payload.len() < 8 {
    ...
    let row_data_start = 4usize
        .checked_add(offsets_len.checked_mul(4).ok_or_else(|| SoraReadError::new("Sora row offsets overflow"))?)
...
fn read_section_payload<'a>(
    ...
    match section.compression {
        ...
        COMPRESSION_ZSTD => {
            let payload = zstd::bulk::decompress(compressed, section.uncompressed_len)
                .map_err(|error| SoraReadError::new(format!("zstd decompression failed: {}", error)))?;
...
fn decode_string_table(payload: &[u8]) -> Result<Vec<std::sync::Arc<str>>, SoraReadError> {
    ...
    for _ in 0..count {
        ...
        if end > payload.len() {
            return Err(SoraReadError::new("Sora string exceeds string table section"));
        }
    ...
    if cursor != payload.len() {
        return Err(SoraReadError::new("Sora string table section has trailing bytes"));
    }
    Ok(strings)
}
...
fn system_time_from_millis(millis: i64) -> std::time::SystemTime {
    if millis >= 0 {
...
fn system_time_to_millis(value: &std::time::SystemTime) -> Result<i64, SoraReadError> {
    match value.duration_since(std::time::UNIX_EPOCH) {
        Ok(duration) => i64::try_from(duration.as_millis())
            .map_err(|_| SoraReadError::new("datetime is too far after unix epoch")),
        Err(error) => {
            let millis = i64::try_from(error.duration().as_millis())
                .map_err(|_| SoraReadError::new("datetime is too far before unix epoch"))?;
            Ok(-millis)
        }
    }
}

impl SoraDecode for std::time::SystemTime {
    fn decode(reader: &mut SoraReader<'_>) -> Result<Self, SoraReadError> {
        Ok(system_time_from_millis(reader.read_i64()?))
    }
}

pub mod serde_system_time_millis {
    pub fn serialize<S>(
    ...
    where
        S: serde::Serializer,
    {
        let millis = super::system_time_to_millis(value).map_err(serde::ser::Error::custom)?;
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        Ok(super::system_time_from_millis(millis))
    }
}
...
fn read_len_prefixed_utf8<'a>(bytes: &'a [u8], cursor: &mut usize) -> Result<&'a str, SoraReadError> {
