...
pub struct DivergentUniverseCoverage {
    #[serde(rename = "id")]
    pub id: i32,
    ...
    pub blocking_gap_ids: Option<Vec<i32>>,
...
impl super::runtime::SoraDecode for DivergentUniverseCoverage {
    fn decode(reader: &mut super::runtime::SoraReader<'_>) -> Result<Self, super::runtime::SoraReadError> {
        Ok(Self {
            ...
            blocking_gap_ids: match reader.read_u8()? { 0 => None, 1 => Some({ let len = reader.read_var_u32()? as usize; let mut values = Vec::with_capacity(len); for _ in 0..len { values.push(<i32 as super::runtime::SoraDecode>::decode(reader)?); } values }), value => return Err(super::runtime::SoraReadError::new(format!("invalid option presence {}", value))), },
...
impl DivergentUniverseCoverage {
...
#[derive(Debug, Clone)]
pub struct DivergentUniverseCoverageTable {
    keys: Vec<i32>,
    rows: SoraMap<i32, DivergentUniverseCoverage>,
    by_stable_key: SoraMap<String, i32>,
}

impl DivergentUniverseCoverageTable {
    pub const NAME: &'static str = "DivergentUniverseCoverage";
    pub const INFO: super::SoraTableInfo = super::SoraTableInfo {
        name: Self::NAME,
        row_type: "DivergentUniverseCoverage",
    ...
    pub(super) fn from_rows(rows: Vec<DivergentUniverseCoverage>) -> Result<Self, super::runtime::SoraReadError> {
    ...
    pub fn get(&self, key: &i32) -> Option<&DivergentUniverseCoverage> {
        self.rows.get(key)
    }
    ...
    pub fn ordered_rows(&self) -> impl Iterator<Item = &DivergentUniverseCoverage> {
        self.keys.iter().filter_map(|key| self.rows.get(key))
    }
    pub fn get_by_stable_key(&self, stable_key: &str) -> Option<&DivergentUniverseCoverage> {
        self.by_stable_key.get(stable_key).and_then(|key| self.rows.get(key))
    }
}

impl std::ops::Deref for DivergentUniverseCoverageTable {
    type Target = SoraMap<i32, DivergentUniverseCoverage>;
...
impl super::ErasedSoraTable for DivergentUniverseCoverageTable {
...
impl super::SoraKeyedTable for DivergentUniverseCoverageTable {
    type Key = i32;
    type Row = DivergentUniverseCoverage;
