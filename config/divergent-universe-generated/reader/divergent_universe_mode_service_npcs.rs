...
pub struct DivergentUniverseModeServiceNpcs {
    #[serde(rename = "id")]
    pub id: i32,
...
impl super::runtime::SoraDecode for DivergentUniverseModeServiceNpcs {
...
impl DivergentUniverseModeServiceNpcs {
...
#[derive(Debug, Clone)]
pub struct DivergentUniverseModeServiceNpcsTable {
    keys: Vec<i32>,
    rows: SoraMap<i32, DivergentUniverseModeServiceNpcs>,
    by_stable_key: SoraMap<String, i32>,
}

impl DivergentUniverseModeServiceNpcsTable {
    pub const NAME: &'static str = "DivergentUniverseModeServiceNpcs";
    pub const INFO: super::SoraTableInfo = super::SoraTableInfo {
        name: Self::NAME,
        row_type: "DivergentUniverseModeServiceNpcs",
    ...
    pub(super) fn from_rows(rows: Vec<DivergentUniverseModeServiceNpcs>) -> Result<Self, super::runtime::SoraReadError> {
    ...
    pub fn get(&self, key: &i32) -> Option<&DivergentUniverseModeServiceNpcs> {
        self.rows.get(key)
    }
    ...
    pub fn ordered_rows(&self) -> impl Iterator<Item = &DivergentUniverseModeServiceNpcs> {
        self.keys.iter().filter_map(|key| self.rows.get(key))
    }
    pub fn get_by_stable_key(&self, stable_key: &str) -> Option<&DivergentUniverseModeServiceNpcs> {
        self.by_stable_key.get(stable_key).and_then(|key| self.rows.get(key))
    }
}

impl std::ops::Deref for DivergentUniverseModeServiceNpcsTable {
    type Target = SoraMap<i32, DivergentUniverseModeServiceNpcs>;
...
impl super::ErasedSoraTable for DivergentUniverseModeServiceNpcsTable {
...
impl super::SoraKeyedTable for DivergentUniverseModeServiceNpcsTable {
    type Key = i32;
    type Row = DivergentUniverseModeServiceNpcs;
