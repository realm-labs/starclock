...
pub struct DivergentUniverseCurioStates {
    #[serde(rename = "id")]
    pub id: i32,
...
impl super::runtime::SoraDecode for DivergentUniverseCurioStates {
...
impl DivergentUniverseCurioStates {
...
#[derive(Debug, Clone)]
pub struct DivergentUniverseCurioStatesTable {
    keys: Vec<i32>,
    rows: SoraMap<i32, DivergentUniverseCurioStates>,
    by_stable_key: SoraMap<String, i32>,
}

impl DivergentUniverseCurioStatesTable {
    pub const NAME: &'static str = "DivergentUniverseCurioStates";
    pub const INFO: super::SoraTableInfo = super::SoraTableInfo {
        name: Self::NAME,
        row_type: "DivergentUniverseCurioStates",
    ...
    pub(super) fn from_rows(rows: Vec<DivergentUniverseCurioStates>) -> Result<Self, super::runtime::SoraReadError> {
    ...
    pub fn get(&self, key: &i32) -> Option<&DivergentUniverseCurioStates> {
        self.rows.get(key)
    }
    ...
    pub fn ordered_rows(&self) -> impl Iterator<Item = &DivergentUniverseCurioStates> {
        self.keys.iter().filter_map(|key| self.rows.get(key))
    }
    pub fn get_by_stable_key(&self, stable_key: &str) -> Option<&DivergentUniverseCurioStates> {
        self.by_stable_key.get(stable_key).and_then(|key| self.rows.get(key))
    }
}

impl std::ops::Deref for DivergentUniverseCurioStatesTable {
    type Target = SoraMap<i32, DivergentUniverseCurioStates>;
...
impl super::ErasedSoraTable for DivergentUniverseCurioStatesTable {
...
impl super::SoraKeyedTable for DivergentUniverseCurioStatesTable {
    type Key = i32;
    type Row = DivergentUniverseCurioStates;
