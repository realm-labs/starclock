// This file was generated by Sora. Do not edit manually.
use super::SoraMap;
use super::universe_mechanic_rule_kind::UniverseMechanicRuleKind;

#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
pub struct UniverseMechanicRule {
    #[serde(rename = "id")]
    pub id: i32,
    #[serde(rename = "stable_key")]
    pub stable_key: String,
    #[serde(rename = "source_record_stable_key")]
    pub source_record_stable_key: String,
    #[serde(rename = "source_file")]
    pub source_file: String,
    #[serde(rename = "rule_kind")]
    pub rule_kind: UniverseMechanicRuleKind,
    #[serde(rename = "native_handler_stable_key")]
    pub native_handler_stable_key: Option<String>,
    #[serde(rename = "source_binding_key")]
    pub source_binding_key: Option<String>,
    #[serde(rename = "parameter_values_json")]
    pub parameter_values_json: String,
    #[serde(rename = "mechanic_tags")]
    pub mechanic_tags: Option<Vec<String>>,
    #[serde(rename = "approximation_replacement_condition")]
    pub approximation_replacement_condition: Option<String>,
    #[serde(rename = "name_en")]
    pub name_en: String,
    #[serde(rename = "name_zh_cn")]
    pub name_zh_cn: String,
    #[serde(rename = "summary_en")]
    pub summary_en: String,
    #[serde(rename = "summary_zh_cn")]
    pub summary_zh_cn: String,
}

impl super::runtime::SoraDecode for UniverseMechanicRule {
    fn decode(
        reader: &mut super::runtime::SoraReader<'_>,
    ) -> Result<Self, super::runtime::SoraReadError> {
        Ok(Self {
            id: <i32 as super::runtime::SoraDecode>::decode(reader)?,
            stable_key: <String as super::runtime::SoraDecode>::decode(reader)?,
            source_record_stable_key: <String as super::runtime::SoraDecode>::decode(reader)?,
            source_file: <String as super::runtime::SoraDecode>::decode(reader)?,
            rule_kind: <UniverseMechanicRuleKind as super::runtime::SoraDecode>::decode(reader)?,
            native_handler_stable_key: match reader.read_u8()? {
                0 => None,
                1 => Some(<String as super::runtime::SoraDecode>::decode(reader)?),
                value => {
                    return Err(super::runtime::SoraReadError::new(format!(
                        "invalid option presence {}",
                        value
                    )));
                }
            },
            source_binding_key: match reader.read_u8()? {
                0 => None,
                1 => Some(<String as super::runtime::SoraDecode>::decode(reader)?),
                value => {
                    return Err(super::runtime::SoraReadError::new(format!(
                        "invalid option presence {}",
                        value
                    )));
                }
            },
            parameter_values_json: <String as super::runtime::SoraDecode>::decode(reader)?,
            mechanic_tags: match reader.read_u8()? {
                0 => None,
                1 => Some({
                    let len = reader.read_var_u32()? as usize;
                    let mut values = Vec::with_capacity(len);
                    for _ in 0..len {
                        values.push(<String as super::runtime::SoraDecode>::decode(reader)?);
                    }
                    values
                }),
                value => {
                    return Err(super::runtime::SoraReadError::new(format!(
                        "invalid option presence {}",
                        value
                    )));
                }
            },
            approximation_replacement_condition: match reader.read_u8()? {
                0 => None,
                1 => Some(<String as super::runtime::SoraDecode>::decode(reader)?),
                value => {
                    return Err(super::runtime::SoraReadError::new(format!(
                        "invalid option presence {}",
                        value
                    )));
                }
            },
            name_en: <String as super::runtime::SoraDecode>::decode(reader)?,
            name_zh_cn: <String as super::runtime::SoraDecode>::decode(reader)?,
            summary_en: <String as super::runtime::SoraDecode>::decode(reader)?,
            summary_zh_cn: <String as super::runtime::SoraDecode>::decode(reader)?,
        })
    }
}

impl UniverseMechanicRule {
    pub(super) fn collect_text_keys<'a>(&'a self, out: &mut Vec<&'a super::runtime::TextKey>) {
        let _ = &out;
    }
}

#[derive(Debug, Clone)]
pub struct UniverseMechanicRuleTable {
    keys: Vec<i32>,
    rows: SoraMap<i32, UniverseMechanicRule>,
    by_stable_key: SoraMap<String, i32>,
}

impl UniverseMechanicRuleTable {
    pub const NAME: &'static str = "UniverseMechanicRule";
    pub const INFO: super::SoraTableInfo = super::SoraTableInfo {
        name: Self::NAME,
        row_type: "UniverseMechanicRule",
        shape: super::SoraTableShape::Keyed,
        primary_key: Some(super::SoraKeyInfo {
            name: "id",
            ty: "i32",
        }),
        indexes: &[super::SoraIndexInfo {
            name: "by_stable_key",
            unique: true,
            fields: &["stable_key"],
        }],
    };

    pub(super) fn from_rows(
        rows: Vec<UniverseMechanicRule>,
    ) -> Result<Self, super::runtime::SoraReadError> {
        let keys = rows.iter().map(|row| row.id).collect::<Vec<_>>();
        let by_stable_key =
            super::build_unique_map_index(rows.iter(), |row| row.stable_key.clone(), |row| row.id);
        let rows = super::decode_map_table(rows, |row| row.id);
        Ok(Self {
            keys,
            rows,
            by_stable_key,
        })
    }

    pub fn get(&self, key: &i32) -> Option<&UniverseMechanicRule> {
        self.rows.get(key)
    }

    pub fn keys(&self) -> &[i32] {
        &self.keys
    }

    pub fn ordered_rows(&self) -> impl Iterator<Item = &UniverseMechanicRule> {
        self.keys.iter().filter_map(|key| self.rows.get(key))
    }
    pub fn get_by_stable_key(&self, stable_key: &str) -> Option<&UniverseMechanicRule> {
        self.by_stable_key
            .get(stable_key)
            .and_then(|key| self.rows.get(key))
    }
}

impl std::ops::Deref for UniverseMechanicRuleTable {
    type Target = SoraMap<i32, UniverseMechanicRule>;

    fn deref(&self) -> &Self::Target {
        &self.rows
    }
}

impl super::ErasedSoraTable for UniverseMechanicRuleTable {
    fn info(&self) -> &'static super::SoraTableInfo {
        &Self::INFO
    }

    fn len(&self) -> usize {
        self.rows.len()
    }
}

impl super::SoraKeyedTable for UniverseMechanicRuleTable {
    type Key = i32;
    type Row = UniverseMechanicRule;

    fn get(&self, key: &Self::Key) -> Option<&Self::Row> {
        self.rows.get(key)
    }

    fn keys(&self) -> &[Self::Key] {
        &self.keys
    }
}
