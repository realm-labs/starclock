// This file was generated by Sora. Do not edit manually.
use super::SoraMap;

#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
pub struct UniverseBlessing {
    #[serde(rename = "id")]
    pub id: i32,
    #[serde(rename = "stable_key")]
    pub stable_key: String,
    #[serde(rename = "path_id")]
    pub path_id: i32,
    #[serde(rename = "rarity")]
    pub rarity: i32,
    #[serde(rename = "name_en")]
    pub name_en: String,
    #[serde(rename = "name_zh_cn")]
    pub name_zh_cn: String,
    #[serde(rename = "summary_en")]
    pub summary_en: String,
    #[serde(rename = "summary_zh_cn")]
    pub summary_zh_cn: String,
    #[serde(rename = "pool_tags")]
    pub pool_tags: Option<Vec<String>>,
    #[serde(rename = "mechanic_tags")]
    pub mechanic_tags: Option<Vec<String>>,
    #[serde(rename = "rule_stable_key")]
    pub rule_stable_key: String,
    #[serde(rename = "source_description_sha256_en")]
    pub source_description_sha256_en: String,
    #[serde(rename = "source_description_sha256_zh_cn")]
    pub source_description_sha256_zh_cn: String,
}

impl super::runtime::SoraDecode for UniverseBlessing {
    fn decode(
        reader: &mut super::runtime::SoraReader<'_>,
    ) -> Result<Self, super::runtime::SoraReadError> {
        Ok(Self {
            id: <i32 as super::runtime::SoraDecode>::decode(reader)?,
            stable_key: <String as super::runtime::SoraDecode>::decode(reader)?,
            path_id: <i32 as super::runtime::SoraDecode>::decode(reader)?,
            rarity: <i32 as super::runtime::SoraDecode>::decode(reader)?,
            name_en: <String as super::runtime::SoraDecode>::decode(reader)?,
            name_zh_cn: <String as super::runtime::SoraDecode>::decode(reader)?,
            summary_en: <String as super::runtime::SoraDecode>::decode(reader)?,
            summary_zh_cn: <String as super::runtime::SoraDecode>::decode(reader)?,
            pool_tags: match reader.read_u8()? {
                0 => None,
                1 => Some({
                    let len = reader.read_var_u32()? as usize;
                    let mut values = Vec::with_capacity(len);
                    for _ in 0..len {
                        values.push(<String as super::runtime::SoraDecode>::decode(reader)?);
                    }
                    values
                }),
                value => {
                    return Err(super::runtime::SoraReadError::new(format!(
                        "invalid option presence {}",
                        value
                    )));
                }
            },
            mechanic_tags: match reader.read_u8()? {
                0 => None,
                1 => Some({
                    let len = reader.read_var_u32()? as usize;
                    let mut values = Vec::with_capacity(len);
                    for _ in 0..len {
                        values.push(<String as super::runtime::SoraDecode>::decode(reader)?);
                    }
                    values
                }),
                value => {
                    return Err(super::runtime::SoraReadError::new(format!(
                        "invalid option presence {}",
                        value
                    )));
                }
            },
            rule_stable_key: <String as super::runtime::SoraDecode>::decode(reader)?,
            source_description_sha256_en: <String as super::runtime::SoraDecode>::decode(reader)?,
            source_description_sha256_zh_cn: <String as super::runtime::SoraDecode>::decode(
                reader,
            )?,
        })
    }
}

impl UniverseBlessing {
    pub(super) fn collect_text_keys<'a>(&'a self, out: &mut Vec<&'a super::runtime::TextKey>) {
        let _ = &out;
    }
}

#[derive(Debug, Clone)]
pub struct UniverseBlessingTable {
    keys: Vec<i32>,
    rows: SoraMap<i32, UniverseBlessing>,
    by_stable_key: SoraMap<String, i32>,
}

impl UniverseBlessingTable {
    pub const NAME: &'static str = "UniverseBlessing";
    pub const INFO: super::SoraTableInfo = super::SoraTableInfo {
        name: Self::NAME,
        row_type: "UniverseBlessing",
        shape: super::SoraTableShape::Keyed,
        primary_key: Some(super::SoraKeyInfo {
            name: "id",
            ty: "i32",
        }),
        indexes: &[super::SoraIndexInfo {
            name: "by_stable_key",
            unique: true,
            fields: &["stable_key"],
        }],
    };

    pub(super) fn from_rows(
        rows: Vec<UniverseBlessing>,
    ) -> Result<Self, super::runtime::SoraReadError> {
        let keys = rows.iter().map(|row| row.id).collect::<Vec<_>>();
        let by_stable_key =
            super::build_unique_map_index(rows.iter(), |row| row.stable_key.clone(), |row| row.id);
        let rows = super::decode_map_table(rows, |row| row.id);
        Ok(Self {
            keys,
            rows,
            by_stable_key,
        })
    }

    pub fn get(&self, key: &i32) -> Option<&UniverseBlessing> {
        self.rows.get(key)
    }

    pub fn keys(&self) -> &[i32] {
        &self.keys
    }

    pub fn ordered_rows(&self) -> impl Iterator<Item = &UniverseBlessing> {
        self.keys.iter().filter_map(|key| self.rows.get(key))
    }
    pub fn get_by_stable_key(&self, stable_key: &str) -> Option<&UniverseBlessing> {
        self.by_stable_key
            .get(stable_key)
            .and_then(|key| self.rows.get(key))
    }
}

impl std::ops::Deref for UniverseBlessingTable {
    type Target = SoraMap<i32, UniverseBlessing>;

    fn deref(&self) -> &Self::Target {
        &self.rows
    }
}

impl super::ErasedSoraTable for UniverseBlessingTable {
    fn info(&self) -> &'static super::SoraTableInfo {
        &Self::INFO
    }

    fn len(&self) -> usize {
        self.rows.len()
    }
}

impl super::SoraKeyedTable for UniverseBlessingTable {
    type Key = i32;
    type Row = UniverseBlessing;

    fn get(&self, key: &Self::Key) -> Option<&Self::Row> {
        self.rows.get(key)
    }

    fn keys(&self) -> &[Self::Key] {
        &self.keys
    }
}
