...
The server exposes seven battle MCP tools over the protocol-neutral
...
numbers remain canonical decimal strings.

| Tool | Input | Structured success result |
|---|---|---|
| `starclock_list_scenarios` | Empty object | The production scenario identities, definition/encounter IDs and default seeds. |
| `starclock_create_battle` | Scenario ID and optional exact decimal seed | Complete first player-visible observation for a newly owned session. |
| `starclock_observe_battle` | Session ID and optional event cursor | Current player-visible observation with the bounded page strictly after the cursor; omission means `event_0`. |
| `starclock_play_action` | Session/decision IDs, expected state hash, opaque action token and idempotency key | Commit/idempotency facts, bounded settlement summary and complete next observation. |
| `starclock_export_replay` | Session ID | Canonical replay as lowercase hex, SHA-256, command count and nonauthoritative controller diagnostics. |
| `starclock_close_battle` | Session ID | Exact closed session identity and `closed:true`; active quota capacity is released. |
| `starclock_verify_replay` | Scenario ID, optional exact seed and lowercase replay hex | Fresh verification command count, final state hash and battle phase. No live session or model is used. |

Optional creation/verification seed values are exact unsigned decimal strings;
omission selects the scenario's
...
| Tool | Input | Structured success result |
|---|---|---|
| `starclock_create_universe` | Optional mode, exact seed and mode entry fields | Complete first external Activity observation for a newly owned session. |
| `starclock_observe_activity` | Session ID | Current external Activity observation. |
| `starclock_play_activity_action` | Session/boundary IDs, expected state hash, opaque action token and idempotency key | Commit/idempotency facts, bounded nested-battle settlement and next observation. |
| `starclock_export_activity_replay` | Session ID | Canonical replay hex, SHA-256, action count and completeness. |
| `starclock_close_activity` | Session ID | Exact closed session identity and `closed:true`; shared Activity quota capacity is released. |
| `starclock_verify_activity_replay` | Optional mode, exact seed, mode entry fields and replay hex | Fresh action/battle counts, final state hash and terminal summary. |
