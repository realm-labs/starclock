...
| Active batch | None |
| Next unblocked batch | `G20-P2-B3` |
...
| Inherited policy boundaries | 30 inherited / 1 terminal / 30 pending |
...
`G20-P2-B2` resolves `topology_policy` as one explicitly labeled
`VersionedExecutablePolicy`; the remaining 30 rows stay `InheritedPolicy`.
