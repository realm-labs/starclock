...
| State | `InProgress` |
| Current phase | Phase 0 — Source correction (`InProgress`) |
| Current batch | `G12-P0-B5` — Correct GridFight selector, source closure, manifest and contracts |
| Next unblocked batch | None until `G12-P0-B5` is committed, pushed and remotely verified |
...
| Blocking condition | Later Phase 1 batches are blocked until `G12-P0-B5` corrects the source denominator and `G12-P1-B10` replaces the superseded Tourn3 flow rows. Goal 11's selector conflict is resolved by the direct GridFight/Tourn3 distinction. |

## Phase ledger

| Phase | State | Evidence |
|---|---|---|
| Phase 0 — Scope, sources and contracts | `InProgress` | Published B1–B4 evidence is immutable, but its Tourn3 source hypothesis is superseded. B5 is freezing the direct GridFight selector, 1,137-path source closure and regenerated denominator/contracts. |
| Phase 1 — Unique mode systems | `InProgressBlockedByCorrection` | B2's public-text Squad HP/action-value facts remain valid. B1's Tourn3/Persona flow rows are historical only and must be replaced by GridFight flow in P1-B10 before P1-B3 resumes. |
...
| `G12-P0-B5` | `InProgress` | — | Corrected the authoritative selector to GuideType `GridFight`; reproduced all 1,137 GridFight paths; generated a 3,822-file inventory and a 19,250-obligation exact-once manifest; regenerated the 96-family/three-workbook/28-fixture contracts and refreshed the still-valid six-row Squad-boundary provenance binding (`f5b15ea1…ee9ba`); preserved all published Tourn3 evidence as immutable history. Awaiting final gates, atomic commit, push and remote SHA verification. |
...
| `G12-P1-B10` | `Pending` | — | Replace superseded Tourn3/Persona flow rows with exact GridFight stages, routes, nodes, difficulties and transitions. |
...
|---|---:|---:|---:|---|
| Profiles/Gambit modes/entries/finish conditions | 111 | 0 | 0 | Includes both direct Guide selectors, two released Gambit policy obligations and all GridFight entry/module/unlock/terminal rows. |
| Planes/difficulties/ranks/Nodes/rooms | 1,327 | 0 | 0 | Includes every GridFight division, route, node, level and tutorial topology row. |
| Squad HP/action-value projections | 129 | 0 | 0 | Includes all GridFight Stage and penalty rows. B2 public facts remain evidence inputs but do not close corrected row obligations. |
...
| `G12-R03` | `OpenCorrection` | What are the exact Standard/Overclock entry, difficulty/rank, three-Plane, Node, transition, finish and carry/reset boundaries? | P1-B10 / P1-B9 / P2-B5 | Replace all Tourn3/Persona flow rows with GridFight stage/route/node closure and exact or policy-bound lifecycle evidence. |
