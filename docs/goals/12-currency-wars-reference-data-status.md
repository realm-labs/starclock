...
| State | `InProgress` |
| Current phase | Phase 0 — Scope, sources and contracts (`Complete` after publication) |
| Current batch | None; `G12-P0-B4` awaits publication of this ledger row |
| Next unblocked batch | `G12-P1-B1`, only after the `G12-P0-B4` commit is pushed and remotely resolved |
...
| Blocking condition | Phase 1 execution is unblocked after `G12-P0-B4` publication; the Goal 11 selector conflict is recorded and blocks terminal `G12-P4-B3` reconciliation unless replaced by a merge-stage ownership decision |
