...
| Current batch | `G12-P1-B2` — Squad HP, action-value and battle-result projection |
| Next unblocked batch | None until `G12-P1-B2` is committed, pushed and remotely verified |
...
| Blocking condition | Phase 1 execution is unblocked after `G12-P1-B1` publication; the Goal 11 selector conflict is recorded and blocks terminal `G12-P4-B3` reconciliation unless replaced by a merge-stage ownership decision |

## Phase ledger

| Phase | State | Evidence |
|---|---|---|
| Phase 0 — Scope, sources and contracts | `Complete` | Foundation, deterministic 2,685-file source closure, 5,283 exact-once obligations, 96 normalized file families, three isolated workbooks, 28 semantic fixture families and four ownership checkpoints are machine-frozen. Pending joins remain evidence-only; one Goal 11 selector conflict is preserved for merge coordination. |
| Phase 1 — Unique mode systems | `InProgress` | Currency Wars identity, Standard/Overclock Gambits, Tourn3 areas/difficulties/layers, sixty exact Persona Nodes, Domain-composition seeds, finish conditions, Squad HP/action-value/result projection and policy-bound carry/reset are normalized. Economy, roster/shop/star, positioning/Empowerment, Bonds, builds, remaining Persona and rank data remain. |
...
|---|---:|---:|---:|---|
| Profiles/Gambit modes/entries/finish conditions | 19 | 19 | 16 | Both Gambit identities and high-level rules are exact public text; the profile and two Gambits remain `Researched` until initial resources close in P1-B3. |
| Planes/difficulties/ranks/Nodes/rooms | 911 | 62 | 40 | Area group, 28 areas, 22 difficulties and 11 layer layouts are accounted; difficulties remain `Researched` until P1-B9 enemy scaling. The 848 room candidates and one rank envelope remain fail-closed/deferred; sixty exact Nodes are derived rows outside this denominator. |
| Squad HP/action-value projections | 1 | 1 | 0 | The parent envelope is accounted by six rows. Initial `100`, victory preservation, non-victory loss, zero failure and unlimited low-difficulty action value are exact; node limits/loss amounts and same-boundary timing remain policy-bound, so the obligation is not yet DataReady. |
...
| Investment Environment/Strategy/Persona | 547 | 96 | 96 | Sixty LayerRoom Nodes, 34 room presets and two composition-list constants are accounted; remaining Persona rows belong to P1-B8. |
...
| `G12-R03` | `OpenNarrowed` | What are the exact Standard/Overclock entry, difficulty/rank, three-Plane, Node/room, transition, finish and carry/reset boundaries? | P1-B9 / P2-B5 | Currency Wars/Gambit names, 28 areas, 22 difficulties, 11 layers, 60 Nodes and 13 finish rows are exact. Replace the area-to-Gambit and carry/reset policies plus the 848 pending room/Stage obligations with released selectors, Stage closure or reproducible observations. |
