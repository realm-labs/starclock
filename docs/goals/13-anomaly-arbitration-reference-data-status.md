...
| Active batch | `G13-P1-B5` complete pending this commit's publication |
| Next unblocked batch | `G13-P1-B6` after remote verification |
...
| `G13-P1-B5` | `Complete` | This row's containing commit | Imported all three active Quadrant options and three selection/no-selection/teardown rules. Selector order is 3033066, 3033068, 3033067; bilingual texts, canonical parameters and binding keys are exact. Plugin 0014 resolves, while layout-named 0022/0023 bodies are absent from the extracted fixed ability list and remain explicit non-runtime gaps. Official text proves select-one-before-King; no-selection rejection and attempt teardown are replaceable policies. Digests: options `555ef28a…357e`; selections `5dad4e83…9374`. Publication contract: `remote=origin`; `branch=codex/goal13-anomaly-arbitration-reference`; push `git push origin HEAD:refs/heads/codex/goal13-anomaly-arbitration-reference`; verify local, tracking and `git ls-remote --exit-code origin refs/heads/codex/goal13-anomaly-arbitration-reference` resolve to the same full commit before P1-B6. |
...
|---|---:|---:|---:|---|
| Profile/period/entry/terminal outcomes | 20 | 13 | 13 | Profile, active period, four entry locators, three exact cycle-limit constants and four terminal outcomes are DataReady; seven lifecycle/retention constants remain. |
...
| `G13-R06` | `InProgress` | Which Arbitral Quadrant rows are offered in the active period, how are they selected and when do their battle contributions start and end? | P1-B5 / P2-B3 | P1-B5 resolved the exact three-row selector, parameters and attempt-local policy; P2-B3 must bind contribution evidence and either recover layout-named plugin bodies 0022/0023 or retain their explicit released-source boundary. |
