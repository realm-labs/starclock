...
| Active batch | `G13-P1-B4` complete pending this commit's publication |
| Next unblocked batch | `G13-P1-B5` after remote verification |
...
| `G13-P1-B4` | `Complete` | This row's containing commit | Imported nine ordered stage-clock rules and three exact turn-limit constants: Knight 6, normal King 6 and Plight 2. Official released text proves independent stage limits, failure on exceed, larger first-cycle action value, cross-wave countdown carry and a low-cycle allied buff. The unavailable first-cycle number, warning threshold, buff identity/parameters and retry clock allocation remain symbolic or explicit field-level policy; no numeric or combat-effect value was invented. Digest: clocks `1d2eaf74…b18d`. Publication contract: `remote=origin`; `branch=codex/goal13-anomaly-arbitration-reference`; push `git push origin HEAD:refs/heads/codex/goal13-anomaly-arbitration-reference`; verify local, tracking and `git ls-remote --exit-code origin refs/heads/codex/goal13-anomaly-arbitration-reference` resolve to the same full commit before P1-B5. |
