...
| Active batch | `G13-P1-B3` complete pending this commit's publication |
| Next unblocked batch | `G13-P1-B4` after remote verification |
...
| `G13-P1-B3` | `Complete` | This row's containing commit | Imported normal and Plight King states plus four ordered protection/shortcut rules, accounting for all six transition obligations. Exact released facts are limited to Knight protection, cutting all three transmissions, distinct normal/Plight StageConfigs and direct Plight three-star equivalence. Numeric protection remains unavailable; per-Knight composition/reset and absence of synthetic loadout snapshots are explicit field-level policies with alternatives and replacement triggers. Digests: states `4f3c5954…d9f0`; protection `e6ac6da4…8e62`. Publication contract: `remote=origin`; `branch=codex/goal13-anomaly-arbitration-reference`; push `git push origin HEAD:refs/heads/codex/goal13-anomaly-arbitration-reference`; verify local, tracking and `git ls-remote --exit-code origin refs/heads/codex/goal13-anomaly-arbitration-reference` resolve to the same full commit before P1-B4. |
