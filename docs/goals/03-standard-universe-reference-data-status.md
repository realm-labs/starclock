...
| `G03-P2-B4` | `Complete` | This row's containing commit | Added source, content-audit, coverage, review-fixture and pack-file Sora tables with typed provenance references and explicit Standard/DLC ownership and quality enums. Generated and committed an isolated 49-table Universe schema lock, 49 Rust readers and three grouped workbook templates alongside the unchanged 82-table core project. The dedicated verifier checks pinned Sora, workbook ownership, canonical-decimal transport, key typed references, schema-lock drift and formatted reader drift. |
| `G03-P2-B5` | `Complete` | This row's containing commit | Added a disposable no-overwrite `openpyxl` fixture writer and automated proof for empty and representative workbook sets. Pinned Sora exports binary/debug output, duplicate representative generation is byte-identical, and the isolated generated-reader acceptance tool parses all 49 Universe tables while asserting representative Profile/Activity/evidence rows. The P4 integration audit separated this staging bundle from the Goal 01 runtime bundle before release. |
...
| `G03-P3-B6` | `Complete` | This row's containing commit | Published the three production `openpyxl` workbooks and regenerated the authoritative isolated Sora bundle/debug output. Two clean authoring/export runs reproduce byte-identical workbooks and the separated bundle (`0d94d25b…3b04`); the workbook verifier closes 13,793 rows, 49 sheets and 86 Goal 01 enemy stable keys with semantic digest `d54c6c1b…9390`. Automated QA enforces metadata, ordering, widths, wrapping, filters, frozen panes and formula/error exclusion. Excel exported all 49 sheets to PDF and the rendered PNG review found no clipping, overlap or rendering defects. Generated Rust readers load all staged Universe tables without admitting them to the combat runtime. |
