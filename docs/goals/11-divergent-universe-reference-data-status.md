...
| Active batch | None |
| Next unblocked batch | `G11-P1-B4` after `G11-P1-B3` is pushed and remotely verified |
...
| Phase 1 — Unique mode systems | `InProgress` | Version 4.4 entry/module, stage closure, policy-bound carry/reset, Arithmetic Mapping and all 80 current Equation definitions/recipes/effects are normalized. Divergent Blessings, Curios/Grand Miracles/Hex, Golden Blood/Titan, protocols, services and progression remain. |
...
| `G11-R05` | `InProgress` | How are Equations offered, rerolled, progressed, expanded, replaced and evaluated when Blessing ownership changes? | P1-B3–B4 | P1-B3 freezes all current definitions, recipes, effect bindings and fail-closed RandomIDs. Offer membership/weights and service timing remain `Unspecified`; P1-B4 must close enhanced/rewrite contribution and ownership-change semantics. |
