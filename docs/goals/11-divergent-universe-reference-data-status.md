...
| Active batch | None |
| Next unblocked batch | `G11-P1-B7` after `G11-P1-B6` is pushed and remotely verified |
...
| Phase 1 — Unique mode systems | `InProgress` | Version 4.4 entry/module, stage closure, policy-bound carry/reset, Arithmetic Mapping, all 80 current Equations, all 414 current Blessings/828 levels, 179 Curio identities/235 copies, 17 Grand Miracles, 12 Titan types, 84 Golden Blood's Boons and 36 permanent Titan talent levels are normalized. Unavailable lifecycle/choice fields are explicit. Protocols, services and progression remain. |
...
| Curios/Weighted Curios/states | 700 | 700 | 179 | All identities/copies/groups are accounted; identities are exact, while 235 copies and 286 groups remain non-DataReady until charges/lifecycle and pool membership/weights are proven. |
...
| `G11-R06` | `ResolvedPolicyBound` | What are the exact Weighted Curio and Grand Miracle/Hex eligibility, weighting, activation, simultaneous-trigger and no-legal-target semantics? | P1-B5 / P2-B2 | Current Hex Path/element eligibility is exact; all 57 historical eligibility rows are excluded by stable-ID/module closure. Curio groups expose no candidates/weights, 17 referenced Hex MazeBuff definitions are absent, and Curio/Hex lifecycle ordering is unpublished, so those fields stay empty or `Unspecified` with reject-without-mutation fallback. Reopen in P2-B2 only when released pool/service/config/ability evidence supplies exact candidates, weights, effects or transitions. |
