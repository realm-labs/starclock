...
| Last completed batch | `G01-P1-B3` |
...
| `G01-P1-B3` | `Complete` | This row's containing commit | `node tools/repository-check/run.mjs`; `node tools/repository-check/run.mjs --with-source-cache`; `git diff --check`. Policy evidence: [`repository-checks.json`](../../policy/repository-checks.json), [`generated-drift.json`](../../policy/generated-drift.json), [`repository-check README`](../../tools/repository-check/README.md). | One pinned command verifies the exact dependency/tool inventory, nine-crate dependency direction and shared-lint inheritance, 12 handwritten Rust files against physical-line/facade limits, two explicit reviewed public re-export declarations, eight clean-checkout generated/integrity checks, formatting, Clippy and all-target/all-feature tests. The optional cache gate also reproduced the prepared pack and bound evidence as a ninth check. |
| `G01-P1-B4` | `Pending` | — | — | Pinned Sora 0.3.0 installation and complete capability golden. |
| `G01-P1-B5` | `Pending` | — | — | Windows/Linux/macOS CI and supported target matrix. |
| `G01-P1-B6` | `Pending` | — | — | Shared stable identity/localization/version/provenance/confidence/evidence/decimal schemas. |
| `G01-P1-B7` | `Pending` | — | — | Character, Ability, HitPlan, Trace, Eidolon and Light Cone schemas/fixtures. |
| `G01-P1-B8` | `Pending` | — | — | Effect, Modifier and typed Rule IR schema families/fixtures. |
