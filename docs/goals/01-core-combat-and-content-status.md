...
| Last completed batch | `G01-P2-B2` |
...
| Phase 2 — Deterministic primitives | `InProgress` | B1 implements numeric policy `fixed-i64-6dp-v1` with private fixed-point storage, checked domains and mechanical no-authoritative-float enforcement. B2 adds the complete foundational definition/runtime ID domains and an immutable typed catalog whose private sorted tables, canonical set validation, typed reference checks and stable program-cycle diagnostics are insertion-order invariant; `G01-P2-B3` next. |
