...
Native checkout uses full Git history because completed Goal evidence is
verified from its recorded completion commit/tree. CI does not rerun historical
Goal-specific foundation/release scripts against current source. Current
workspace tests, generated-data validation, protocol vectors and the active
Goal's gates own current compatibility; immutable snapshots preserve the
historical Goal 01–04 claims without freezing additive implementation work.
