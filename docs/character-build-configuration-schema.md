...
Production content remains `.xlsx`-authored. `G01-P1-B10` installs the
workbooks and generated readers with disabled identity staging; later content
batches fill executable rows, and `G01-P1-B11` converts generated rows into
validated domain definitions. No runtime JSON path is authorized.
