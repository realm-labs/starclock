...
fn ultimate_and_skill_resources_gate_offers_and_multi_hit_target_locks() {
    ...
    assert_eq!(
        ...
        [
            16, 77, 146, 186, 122, 209, 246, 86, 141, 238, 253, 46, 142, 196, 89, 249, 117, 107,
            21, 246, 4, 99, 54, 170, 218, 131, 191, 59, 242, 49, 56, 241,
        ]
    );
    assert!(matches!(
    ...
    assert_eq!(
        resolution.state_hash().bytes(),
        [
            96, 89, 16, 215, 189, 243, 69, 86, 234, 51, 245, 132, 119, 26, 101, 59, 26, 67, 210,
            55, 196, 86, 244, 6, 36, 212, 200, 81, 214, 86, 83, 223,
        ]
    );
    assert!(matches!(
        resolution.events()[2].kind(),
