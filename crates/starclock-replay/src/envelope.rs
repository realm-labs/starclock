...
//!
//! The envelope binds the exact runtime components consumed by a replay.
...
pub const REPLAY_ENVELOPE_TAG: u32 = 3;
...
fn decode_header(d: &mut Decoder<'_>) -> Result<ReplayHeader, ReplayError> {
    ...
    if envelope_tag != REPLAY_ENVELOPE_TAG {
        return Err(ReplayError::Format(
            ReplayFormatError::UnexpectedEnvelopeTag(envelope_tag),
        ));
    }
    let schema = d.u32()?;
    if schema != REPLAY_SCHEMA_TAG {
        return Err(ReplayError::Format(ReplayFormatError::UnexpectedSchemaTag(
            schema,
        )));
    }
    let policy = d.u8()?;
...
impl fmt::Display for ReplayError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(formatter, "replay envelope error: {self:?}")
    }
}
