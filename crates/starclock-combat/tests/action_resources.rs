...
fn ultimate_and_skill_resources_gate_offers_and_multi_hit_target_locks() {
    ...
    assert_eq!(
        ...
        [
            107, 187, 167, 48, 242, 90, 217, 217, 212, 111, 61, 73, 79, 20, 18, 101, 15, 163, 197,
            68, 80, 221, 158, 69, 192, 16, 90, 134, 119, 238, 211, 98,
        ]
    );
    assert!(matches!(
    ...
    assert_eq!(
        resolution.state_hash().bytes(),
        [
            247, 40, 52, 202, 203, 141, 220, 128, 208, 154, 105, 218, 13, 88, 169, 242, 77, 2, 191,
            238, 195, 142, 186, 48, 163, 91, 17, 40, 21, 71, 105, 95,
        ]
    );
    assert!(matches!(
        resolution.events()[2].kind(),
