...
fn damage_and_healing_emit_calculated_and_effective_hp_facts() {
    ...
    assert_eq!(
        ...
        [
            108, 28, 89, 128, 189, 74, 152, 207, 110, 203, 173, 202, 117, 161, 99, 253, 57, 30,
            230, 104, 84, 114, 32, 20, 120, 105, 198, 230, 254, 103, 229, 3,
        ]
    );
    let damage = resolution
        .events()
...
fn single_wave_defeat_settles_to_victory_and_terminal_rejection_is_immutable() {
    ...
    assert_eq!(
        resolution.state_hash().bytes(),
        [
            151, 59, 87, 163, 66, 78, 81, 32, 68, 224, 203, 51, 206, 71, 171, 153, 76, 167, 175, 6,
            97, 108, 153, 155, 252, 222, 29, 46, 43, 42, 22, 241,
        ]
    );
    assert_eq!(resolution.phase(), BattlePhase::Won);
    assert!(resolution.next_decision().is_none());
...
fn after_action_wave_transition_does_not_let_later_hits_reach_reserve_units() {
    ...
    assert_eq!(
        ...
        [
            184, 69, 13, 246, 35, 9, 49, 74, 185, 225, 232, 34, 49, 50, 121, 25, 134, 193, 29, 141,
            191, 68, 7, 195, 165, 35, 226, 208, 141, 0, 69, 121,
        ]
    );
    ...
    assert_eq!(
        ...
        [
            78, 203, 58, 155, 76, 38, 149, 51, 27, 240, 178, 181, 146, 130, 121, 168, 59, 46, 69,
            67, 182, 75, 224, 64, 229, 31, 179, 239, 227, 132, 76, 184,
        ]
    );
...
fn defeating_the_last_player_settles_loss() {
    ...
    assert_eq!(
        resolution.state_hash().bytes(),
        [
            143, 141, 2, 196, 217, 111, 120, 109, 23, 118, 246, 227, 196, 10, 76, 247, 134, 108,
            139, 68, 251, 153, 207, 8, 121, 81, 130, 20, 211, 34, 147, 104,
        ]
    );
