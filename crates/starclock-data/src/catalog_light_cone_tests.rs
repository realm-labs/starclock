...
#[test]
fn production_l01_has_complete_curves_ranks_and_compilable_passives() {
    ...
    assert_eq!(builds.light_cone_ids().count(), 16);

    for raw in 112..=127 {
        let id = LightConeId::new(raw).unwrap();
        let cone = builds.light_cone(id).expect("L01 cone must lower");
