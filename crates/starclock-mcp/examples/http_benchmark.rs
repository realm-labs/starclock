...
fn main() {
    assert!(
        std::env::args().len() == 1,
        "http_benchmark takes no arguments"
    );
...
fn measure_observe(runtime: &tokio::runtime::Runtime) -> Row {
    ...
    let created =
        runtime.block_on(client.tool("starclock_create_battle", json!({"scenario_id":SCENARIO})));
        ...
        runtime.block_on(async {
            for _ in 0..OBSERVATIONS {
                let (observed, bytes) = client
                    .tool_measured("starclock_observe_battle", json!({"session_id":session_id}))
                    .await;
...
fn measure_actions(runtime: &tokio::runtime::Runtime) -> Row {
    ...
    for index in 0..SESSIONS {
        let created = runtime
            .block_on(client.tool("starclock_create_battle", json!({"scenario_id":SCENARIO})));
        ...
        runtime.block_on(async {
            for _ in 0..SESSIONS {
                let (created, bytes) = client
                    .tool_measured("starclock_create_battle", json!({"scenario_id":SCENARIO}))
                    .await;
