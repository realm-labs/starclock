...
use crate::{metadata::MCP_PROTOCOL_REVISION, server::StarclockMcp};
use axum::{
    Router,
    body::{Body, to_bytes},
    http::{
        HeaderMap, HeaderValue, Method, Request, Response, StatusCode, Uri,
        header::{ALLOW, CONTENT_LENGTH, CONTENT_TYPE, HOST, RETRY_AFTER, TRANSFER_ENCODING},
    },
...
pub fn loopback_router(config: &LoopbackHttpConfig) -> Result<Router, HttpServeError> {
    ...
    let guard = HttpGuard::new(config);
    Ok(Router::new()
        .nest_service(MCP_HTTP_PATH, service)
        .layer(middleware::from_fn_with_state(guard, guard_request)))
}
...
impl HttpGuard {
    fn new(config: &LoopbackHttpConfig) -> Self {
        Self {
...
async fn guard_request(
    ...
    if has_forwarding_header(request.headers()) {
        return response(
    ...
    if request
        ...
        != Some(guard.authority.as_ref())
    {
        return response(StatusCode::FORBIDDEN, "Host is not allowed");
    }
    if let Some(origin) = request.headers().get("origin") {
        let Ok(origin) = origin.to_str() else {
            return response(StatusCode::FORBIDDEN, "Origin is not allowed");
        };
        if !guard
            .allowed_origins
            .iter()
