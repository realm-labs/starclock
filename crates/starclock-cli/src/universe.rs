...
pub fn config_validate(args: &[String]) -> Result<(), UniverseCliError> {
    ...
    if json {
        println!(
            "{{\"kind\":\"universe-config-validation\",\"valid\":true,\"bundle_sha256\":\"{}\",\"worlds\":{},\"difficulties\":{},\"paths\":{},\"blessings\":{},\"curios\":{}}}",
            hex(digest),
            catalog.worlds().len(),
...
pub fn coverage(args: &[String]) -> Result<(), UniverseCliError> {
    ...
    if json {
        println!(
            "{{\"kind\":\"universe-coverage\",\"content_records\":2201,\"rule_bindings\":786,\"fixtures\":78,\"worlds\":{},\"difficulties\":{},\"paths\":{},\"encounter_groups\":{}}}",
            catalog.worlds().len(),
            catalog.difficulties().len(),
            catalog.paths().len(),
...
pub fn verify_replay(bytes: &[u8], json: bool) -> Result<(), UniverseCliError> {
    ...
    if json {
        println!(
            "{{\"kind\":\"replay-verify\",\"entry\":\"standard-universe\",\"actions\":{},\"nested_battles\":{},\"battle_commands\":{},\"terminal\":\"completed\",\"state_hash\":\"{}\"}}",
            report.action_count(),
            report.battle_count(),
...
fn context(
    ...
    let (profile_id, activity, _materialized_combat_catalog, components, environment) =
