...
pub fn config_validate(args: &[String]) -> Result<(), SwarmDisasterCliError> {
    ...
    if json {
        println!(
            "{{\"kind\":\"universe-config-validation\",\"mode\":\"{MODE}\",\"valid\":true,\"bundle_sha256\":\"{bundle_digest}\",\"tables\":65,\"rows\":33380,\"source_obligations\":6963,\"mechanic_rules\":23,\"fixtures\":23,\"policy_boundaries\":31}}"
        );
    } else {
        println!(
...
pub fn verify_replay(bytes: &[u8], json: bool) -> Result<(), SwarmDisasterCliError> {
    ...
    if json {
        println!(
            "{{\"kind\":\"replay-verify\",\"entry\":\"swarm-disaster\",\"actions\":{},\"nested_battles\":{},\"battle_commands\":{},\"terminal\":\"completed\",\"state_hash\":\"{}\"}}",
            report.action_count(),
            report.battle_count(),
            report.battle_command_count(),
            hex(report.final_state_hash().bytes()),
