...
pub fn config_validate(args: &[String]) -> Result<(), GoldAndGearsCliError> {
    ...
    if json {
        println!(
            "{{\"kind\":\"universe-config-validation\",\"mode\":\"{MODE}\",\"valid\":true,\"bundle_sha256\":\"{}\",\"tables\":{},\"rows\":{},\"source_obligations\":{},\"mechanic_rules\":{},\"fixtures\":{},\"policy_boundaries\":{}}}",
...
pub fn verify_replay(bytes: &[u8], json: bool) -> Result<(), GoldAndGearsCliError> {
    ...
    if json {
        println!(
            "{{\"kind\":\"replay-verify\",\"entry\":\"gold-and-gears\",\"actions\":{},\"nested_battles\":{},\"battle_commands\":{},\"terminal\":\"completed\",\"state_hash\":\"{}\"}}",
            report.action_count(),
            report.battle_count(),
