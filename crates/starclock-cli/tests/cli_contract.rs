...
fn config_validation_uses_only_a_validated_sora_bundle() {
    ...
    assert_eq!(
        ...
        "{\"schema_revision\":\"starclock-cli-v1\",\"kind\":\"config-validation\",\"valid\":true,\"game_version\":\"4.4\",\"data_revision\":\"core-combat-v1-phase7-l02\",\"bundle_sha256\":\"b50e74be43d9f6326647102fe9327d2b69209ad0223980f337fda3bacb5303a3\",\"identities\":4577,\"enabled\":4444}"
    );
    ...
    assert!(human.contains("identities=4577 enabled=4444"));
...
fn coverage_is_goal_aware_filterable_and_not_readiness_inflated() {
    ...
    assert!(
        all.contains("\"required\":283,\"enabled\":150,\"data_ready\":150,\"golden_verified\":150")
    );
    for expected in [
    ...
    assert!(filtered.contains("\"data_ready\":32"));
