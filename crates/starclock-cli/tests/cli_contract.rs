...
fn config_validation_uses_only_a_validated_sora_bundle() {
    ...
    assert_eq!(
        ...
        "{\"schema_revision\":\"starclock-cli-v1\",\"kind\":\"config-validation\",\"valid\":true,\"game_version\":\"4.4\",\"data_revision\":\"core-combat-v1-phase7-l09\",\"bundle_sha256\":\"486b686149592765d73d6d761e97c2256857b2a8ecb2739b901b0731612c0631\",\"identities\":5168,\"enabled\":5147}"
    );
    ...
    assert!(human.contains("identities=5168 enabled=5147"));
...
fn coverage_is_goal_aware_filterable_and_not_readiness_inflated() {
    ...
    assert!(
        all.contains("\"required\":283,\"enabled\":262,\"data_ready\":262,\"golden_verified\":262")
    );
    for expected in [
    ...
    assert!(filtered.contains("\"data_ready\":144"));
