...
assert(args.every((argument) => argument === "--bless"), "usage: verify-firefly-damage.mjs [--bless]");
const bless = args.includes("--bless");
const fixture = path.join(root, "config/probes/v1a/firefly-damage");
const work = path.join(root, ".cache/firefly-damage-probe-verify");
assert(path.relative(root, work).replaceAll("\\", "/") === ".cache/firefly-damage-probe-verify", "unexpected work path");
...
assertSameFile(first.bundle, second.bundle, "independent Firefly probe bundles differ");
...
if (bless) {
  ...
  console.log(`Blessed Firefly damage probe (${golden.bundle_sha256}).`);
} else {
  assert(fs.existsSync(bundle) && fs.existsSync(expected), "Firefly damage probe is not blessed");
  assertSameFile(first.bundle, bundle, "committed Firefly damage bundle drifted");
  assert(JSON.stringify(readJson(expected)) === JSON.stringify(golden), "Firefly damage golden drifted");
  console.log(`Firefly damage probe verified (${golden.bundle_sha256}).`);
}

function generate(name) {
  const data = path.join(work, name, "data");
  const out = path.join(work, name, "out");
  run("cargo", ["run", "--manifest-path", "tools/workbook-bootstrap/Cargo.toml", "--locked", "--quiet", "--", "config/generated/templates", "config/probes/v1a/firefly-damage/rows", path.relative(root, data)]);
