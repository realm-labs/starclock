...
const legacyProjectPath = path.join(
  root,
  "config/divergent-universe/project.toml",
);
...
fs.rmSync(legacyProjectPath, { force: true });
...
function renderProject(schemaFiles) {
  return [
    ...
    `includes = ${toml(schemaFiles.map((file) => `divergent-universe/schema/${file}`))}`,
    "",
    "[build]",
    'default_source_format = "xlsx"',
    'data_root = "divergent-universe/data"',
    'view = "default"',
...
function renderSchema(filename, contracts) {
  ...
  if (filename === "core.toml")
    for (const [name, values] of enums()) lines.push(
      "[[enums]]",
      `name = ${quote(name)}`,
      `values = ${toml(values.map((value, id) => ({ id, name: value })))}`,
      "",
    );
  for (const [index, contract] of contracts.entries()) {
    ...
    lines.push(
      "[[tables]]",
      `id = ${quote(contract.file.replace(/\.json$/u, ""))}`,
      `name = ${quote(table)}`,
      'mode = "map"',
      'key = "id"',
      "[tables.source]",
