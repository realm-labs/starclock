...
assert(
  ...
  "usage: node tools/divergent-universe-reference/run-clean-checkout.mjs",
);
assert(
  capture("git", ["diff", "--name-only"]) === "" &&
    capture("git", ["ls-files", "--others", "--exclude-standard"]) === "",
  "stage every proposed Goal 11 file before clean-checkout acceptance",
);
...
const temporary = fs.mkdtempSync(
  path.join(os.tmpdir(), "starclock-divergent-clean-"),
);
...
try {
  ...
  fetchCheckpoint(
    ...
    "refs/goal11-checkpoints/goal08",
  );
  fetchCheckpoint(
    checkout,
  ...
  run(
    ...
    [
      ...
      "--quiet",
      "-m",
      "Goal 11 clean acceptance snapshot",
    ],
    checkout,
  );
  ...
  const python = environment.STARCLOCK_PYTHON;
  const commands = [
    [
      "node",
      "tools/divergent-universe-reference/freeze-reconciliation-checkpoints.mjs",
      checkout,
    ],
    ["node", "tools/divergent-universe-reference/verify-contracts.mjs"],
    ["node", "tools/divergent-universe-reference/audit-release.mjs", checkout],
    [
      "node",
      "tools/divergent-universe-reference/verify-semantic-fixtures.mjs",
      checkout,
    ],
    [
      python,
      "tools/divergent-universe-reference/verify_workbooks.py",
      checkout,
    ],
    [
      "node",
  ...
  for (const command of commands) {
    run(command[0], command.slice(1), checkout, environment);
  }
  assert(
  ...
  console.log(
    `Divergent Universe clean-checkout acceptance passed for staged tree ` +
      `${tree} (fresh build target; no inherited source cache).`,
  );
} finally {
...
function fetchCheckpoint(checkout, commit, reference) {
  run(
    "git",
    ["fetch", "--quiet", "--no-tags", root, `${commit}:${reference}`],
    checkout,
  );
}
