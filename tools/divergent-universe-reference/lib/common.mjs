...
export const ROW_SCHEMA = "starclock.divergent-universe-row.v1";
...
export async function createContext(root, sourceRoot) {
  ...
  function sourceRef(entry, evidenceQuality = "ExactStructured", extra = {}) {
    return {
      source_id:
        `source.goal11.${slug(entry.sourcePath)}.${slug(entry.locator)}`,
  ...
  async function policyRef(locator, note, replacementCondition) {
    ...
    return {
      source_id: `source.goal11.project-policy.${slug(locator)}`,
      repository: "starclock",
      revision: "starclock.divergent-universe-normalized-schema.v1",
      path: relative,
      locator,
      sha256: sha256(bytes),
      access_date: ACCESS_DATE,
