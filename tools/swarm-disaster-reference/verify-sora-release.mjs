...
const committedData = path.join(root, "config", "swarm-disaster", "data");
const python = process.env.STARCLOCK_PYTHON ?? "python3";
const temporary = fs.mkdtempSync(
  path.join(os.tmpdir(), "starclock-swarm-disaster-release-"),
);
const workbooks = [
  "SwarmDisaster.xlsx",
...
try {
  ...
  for (const output of [firstData, secondData])
    run(python, [
      "tools/swarm-disaster-reference/author_workbooks.py",
      "--root",
      root,
      "--output",
      output,
  ...
  for (const workbook of workbooks) {
    assertSame(
      path.join(firstData, workbook),
      path.join(secondData, workbook),
      `${workbook} double generation`,
    );
    assertSame(
      path.join(firstData, workbook),
      path.join(committedData, workbook),
      `${workbook} committed drift`,
    );
  }
  ...
  assertSame(
    firstBundle,
    path.join(committed, "config.sora"),
    "committed Sora bundle drift",
  );
  assertSameTree(firstDebug, secondDebug, "Sora debug double export");
  assertSameTree(
    firstDebug,
    path.join(committed, "debug-json"),
    "committed Sora debug drift",
  );
  ...
  for (const table of tables) {
    const payload = JSON.parse(
      fs.readFileSync(
        path.join(firstDebug, `${table.name}.json`),
        "utf8",
      ),
    );
  ...
  assert(
    tables.length === 65 && rowCount === 33_380,
    "Sora table/row denominator differs",
  );
  run(
    "cargo",
    [
  ...
  console.log(
    `Swarm Disaster Sora release verified (${tables.length} tables, ` +
    `${rowCount} rows; byte-identical workbooks/bundle/debug export; ` +
    "every generated reader loaded).",
  );
} finally {
...
function assertSameTree(first, second, label) {
  ...
  for (const file of firstFiles)
    assertSame(
      path.join(first, file),
      path.join(second, file),
      `${label}/${file}`,
    );
}
