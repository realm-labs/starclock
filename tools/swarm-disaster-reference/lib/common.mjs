...
export const ROW_SCHEMA = "starclock.swarm-disaster-row.v1";
...
export async function createContext(root) {
  ...
  function sourceRef(entry, evidenceQuality = "ExactStructured", extra = {}) {
    return {
      source_id: `source.goal09.${slug(entry.sourcePath)}.${slug(entry.locator)}`,
  ...
  async function policyRef(locator, note, replacementCondition) {
    const relative = "content-manifests/swarm-disaster-v1/normalized-schema.json";
    const bytes = await fs.readFile(path.join(root, relative));
    return {
      source_id: `source.goal09.project-policy.${slug(locator)}`,
      repository: "starclock",
      revision: "starclock.swarm-disaster-normalized-schema.v1",
      path: relative,
      locator,
      sha256: sha256(bytes),
      access_date: ACCESS_DATE,
  ...
  function publicRef({
    ...
    return {
      source_id: `source.goal09.public.${slug(id)}`,
