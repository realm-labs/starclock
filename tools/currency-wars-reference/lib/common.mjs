...
export const ROW_SCHEMA = "starclock.currency-wars-row.v1";
...
export function decimal(value) {
  ...
  const result = text.replace(/0+$/u, "").replace(/\.$/u, "");
...
export async function createContext(root, sourceRoot) {
  const resolvedSourceRoot = path.resolve(
    sourceRoot ?? path.join(root, ".cache/content-reference/turnbasedgamedata"),
  );
  const outputRoot = path.join(root, "content-reference/currency-wars-v1");

  async function readSource(relative) {
    const raw = await fs.readFile(path.join(resolvedSourceRoot, relative), "utf8");
    return JSON.parse(raw.replace(/("Hash"\s*:\s*)(-?\d{16,})/gu, '$1"$2"'));
  }

  async function table(name) {
    const sourcePath = `ExcelOutput/${name}.json`;
    const raw = await fs.readFile(path.join(resolvedSourceRoot, sourcePath), "utf8");
    const evidenceRows = JSON.parse(raw);
    const rows = JSON.parse(
      raw.replace(/("Hash"\s*:\s*)(-?\d{16,})/gu, '$1"$2"'),
    );
    if (!Array.isArray(rows) || !Array.isArray(evidenceRows))
      throw new Error(`expected source array ${sourcePath}`);
    return rows.map((row, index) => ({
      sourcePath,
      locator: String(index),
      row,
      evidenceRow: evidenceRows[index],
    }));
  }
  ...
  function sourceRef(entry, evidenceQuality = "ExactStructured", extra = {}) {
    return {
      source_id:
        `source.goal12.${slug(entry.sourcePath)}.${slug(entry.locator)}`,
      repository: "https://gitlab.com/Dimbreath/turnbasedgamedata.git",
      revision: SOURCE_REVISION,
      path: entry.sourcePath,
      locator: entry.locator,
      sha256: sha256(`${JSON.stringify(entry.evidenceRow)}\n`),
  ...
  function textRef(hashValue, locale) {
    const hash = String(hashValue);
    const isChinese = locale === "zh_cn";
    const rawValue = (isChinese ? textZh : textEn)[hash] ?? "";
    if (!cleanText(rawValue))
      throw new Error(`missing ${locale} TextMap value ${hash}`);
    return {
      source_id: `source.goal12.textmap.${locale}.${hash}`,
      repository: "https://gitlab.com/Dimbreath/turnbasedgamedata.git",
      revision: SOURCE_REVISION,
      path: isChinese
      ...
      locator: hash,
      sha256: sha256(`${JSON.stringify({ [hash]: rawValue })}\n`),
      access_date: ACCESS_DATE,
      game_version: GAME_VERSION,
  ...
  function bilingualTextRefs(hash) {
    return [textRef(hash, "en"), textRef(hash, "zh_cn")];
  }
  ...
  async function policyRef(locator, note, replacementCondition) {
    ...
    return {
      source_id: `source.goal12.project-policy.${slug(locator)}`,
      repository: "starclock",
      revision: "starclock.currency-wars-normalized-schema.v1",
      path: relative,
      locator,
      sha256: sha256(bytes),
      access_date: ACCESS_DATE,
