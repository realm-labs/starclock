...
"Goal 20 release-audit policy identity drift");
assert(Object.values(policy.contracts).every((value) => value === true),
  "every Goal 20 release-audit contract must be enabled");
if (!artifactOnly) {
...
"Goal 20 prior-goal inventory differs from its frozen foundation");
const prior = policy.required_prior_goals.map((goalId) => {
  ...
  `${goalId}: current completion snapshot differs from Goal 20 foundation`);
...
"Goal 20 runtime completeness audit drift");
...
"Goal 20 exact-once release audit has gaps, duplicates or orphans");
...
"Goal 20 introduced or removed a registry package");
...
"Goal 20 generated-drift denominator drift");
assert(nativePolicy.v1a_reviews.length === 8,
  "Goal 20 native-handler scope denominator drift");

const report = {
  schema_revision: "starclock.goal20-release-audits-evidence.v1",
...
console.log(`Goal 20 release audits verified (${prior.length} prior releases, ${protectedRoots.length} protected roots, ${currentRegistry.length} registry packages).`);
