...
#[path = "../../../../config/pure-fiction-generated/readers/rust/mod.rs"]
mod generated;

use generated::{runtime::SoraBundle, SoraConfig};

const EXPECTED_TABLES: usize = 37;
const EXPECTED_ROWS: usize = 6_810;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some((name, _)) = tables
        .iter()
        .find(|(name, rows)| !name.starts_with("Pf") || *rows == 0)
    {
        return Err(format!("{name}: missing or non-Goal-15 table").into());
    }
    ...
    println!(
        "Pure Fiction bundle loaded through every generated reader: \
