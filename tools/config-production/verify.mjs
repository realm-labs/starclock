...
function verifyGeneratedOutput(directory) {
  ...
  assert(counts.get("SourceRecord") === 4 && counts.get("EvidenceRecord") === 5, "production provenance counts differ");
  assert(counts.get("ContentIdentity") === 5383 && counts.get("ContentEvidenceBinding") === 5668 && counts.get("ConfigManifest") === 1, "production identity counts differ");
  for (const [name, expected] of Object.entries({
    Ability: 677, AbilityLevelParameter: 17606, AbilityResourceDelta: 513,
    AiGraph: 23, EnemyTemplate: 19, EnemyVariant: 19, Encounter: 6,
    ...
    Effect: 17, EffectGrantedAbility: 3, EffectModifierBinding: 6, ModifierDefinition: 1561,
    ModifierStackingGroup: 30, ModifierFilter: 151,
    CountdownDefinition: 1, LinkedUnitDefinition: 8,
    Operation: 104, Program: 38, ProgramStep: 108, RuleDefinition: 170, RuleSourceTag: 0, Selector: 80,
    StateSlot: 3, ValueExpression: 1649, LightCone: 165, LightConeStat: 14190,
  ...
  assert(identities.filter((row) => value(row, "enabled") === true).length === 5383, "production enabled identity count differs");
  const coverage = Object.groupBy(identities, (row) => value(row, "coverage_state"));
  assert((coverage.GoldenVerified?.length ?? 0) === 388 && (coverage.DataReady?.length ?? 0) === 4995, "released content coverage states differ");
