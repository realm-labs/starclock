...
function verifyGeneratedOutput(directory) {
  ...
  assert(counts.get("SourceRecord") === 10 && counts.get("EvidenceRecord") === 11, "production provenance counts differ");
  assert(counts.get("ContentIdentity") === 5649 && counts.get("ContentEvidenceBinding") === 5940 && counts.get("ConfigManifest") === 1, "production identity counts differ");
  for (const [name, expected] of Object.entries({
    Ability: 727, AbilityLevelParameter: 17606, AbilityResourceDelta: 513,
    AiGraph: 36, EnemyTemplate: 25, EnemyVariant: 25, Encounter: 6,
    ...
    Effect: 46, EffectGrantedAbility: 3, EffectModifierBinding: 15, ModifierDefinition: 1570,
    ModifierStackingGroup: 39, ModifierFilter: 151,
    CountdownDefinition: 1, LinkedUnitDefinition: 22,
    Operation: 243, Program: 107, ProgramStep: 264, RuleDefinition: 180, RuleSourceTag: 0, Selector: 140,
    StateSlot: 3, ValueExpression: 1773, LightCone: 165, LightConeStat: 14190,
  ...
  assert(identities.filter((row) => value(row, "enabled") === true).length === 5649, "production enabled identity count differs");
  const coverage = Object.groupBy(identities, (row) => value(row, "coverage_state"));
  assert((coverage.GoldenVerified?.length ?? 0) === 654 && (coverage.DataReady?.length ?? 0) === 4995, "released content coverage states differ");
