...
function verifyGeneratedOutput(directory) {
  ...
  assert(counts.get("SourceRecord") === 15 && counts.get("EvidenceRecord") === 16, "production provenance counts differ");
  assert(counts.get("ContentIdentity") === 6122 && counts.get("ContentEvidenceBinding") === 6435 && counts.get("ConfigManifest") === 1, "production identity counts differ");
  for (const [name, expected] of Object.entries({
    Ability: 835, AbilityLevelParameter: 17606, AbilityResourceDelta: 513,
    AiGraph: 69, EnemyTemplate: 47, EnemyVariant: 47, Encounter: 6,
    ...
    Effect: 90, EffectGrantedAbility: 3, EffectModifierBinding: 20, ModifierDefinition: 1575,
    ModifierStackingGroup: 44, ModifierFilter: 151,
    CountdownDefinition: 1, LinkedUnitDefinition: 37,
    Operation: 514, Program: 253, ProgramStep: 558, RuleDefinition: 192, RuleSourceTag: 0, Selector: 203,
    StateSlot: 3, ValueExpression: 1936, LightCone: 165, LightConeStat: 14190,
  ...
  assert(identities.filter((row) => value(row, "enabled") === true).length === 6122, "production enabled identity count differs");
  const coverage = Object.groupBy(identities, (row) => value(row, "coverage_state"));
  assert((coverage.GoldenVerified?.length ?? 0) === 1132 && (coverage.DataReady?.length ?? 0) === 4990, "released content coverage states differ");
