...
function verifyGeneratedOutput(directory) {
  ...
  assert(counts.get("ContentIdentity") === 1475 && counts.get("ContentEvidenceBinding") === 1527 && counts.get("ConfigManifest") === 1, "production identity counts differ");
  for (const [name, expected] of Object.entries({
    Ability: 218, AbilityLevelParameter: 4631, AbilityResourceDelta: 104,
    AiGraph: 17, EnemyTemplate: 17, EnemyVariant: 17, Encounter: 6,
    StandardProfile: 1, StandardScenario: 6, HitPlan: 131,
    Character: 22, CharacterStat: 1892, CharacterResource: 18,
    CharacterAbilityBinding: 150, TraceNode: 411, TracePatch: 224, Eidolon: 132, EidolonPatch: 108,
    Effect: 3, EffectModifierBinding: 1, ModifierDefinition: 221,
    ModifierStackingGroup: 4, ModifierFilter: 53,
    CountdownDefinition: 1, LinkedUnitDefinition: 1,
    Operation: 27, Program: 12, RuleDefinition: 3, Selector: 21,
    StateSlot: 3, ValueExpression: 258,
  ...
  assert(identities.filter((row) => value(row, "enabled") === true).length === 1244, "production enabled identity count differs");
  const coverage = Object.groupBy(identities, (row) => value(row, "coverage_state"));
  assert((coverage.GoldenVerified?.length ?? 0) === 52 && (coverage.DataReady?.length ?? 0) === 1192, "character ready coverage states differ");
