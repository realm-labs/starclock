...
function verifyGeneratedOutput(directory) {
  ...
  assert(counts.get("ContentIdentity") === 5045 && counts.get("ContentEvidenceBinding") === 5291 && counts.get("ConfigManifest") === 1, "production identity counts differ");
  for (const [name, expected] of Object.entries({
    ...
    Effect: 4, EffectGrantedAbility: 3, EffectModifierBinding: 1, ModifierDefinition: 1366,
    ModifierStackingGroup: 21, ModifierFilter: 151,
    CountdownDefinition: 1, LinkedUnitDefinition: 1,
    Operation: 30, Program: 14, ProgramStep: 35, RuleDefinition: 131, RuleSourceTag: 0, Selector: 56,
    StateSlot: 3, ValueExpression: 1406, LightCone: 128, LightConeStat: 11008,
    LightConeSuperimposition: 2005,
  ...
  assert(identities.filter((row) => value(row, "enabled") === true).length === 5008, "production enabled identity count differs");
  const coverage = Object.groupBy(identities, (row) => value(row, "coverage_state"));
  assert((coverage.GoldenVerified?.length ?? 0) === 246 && (coverage.DataReady?.length ?? 0) === 4762, "released content coverage states differ");
