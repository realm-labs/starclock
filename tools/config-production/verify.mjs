...
function verifyGeneratedOutput(directory) {
  ...
  assert(counts.get("SourceRecord") === 12 && counts.get("EvidenceRecord") === 13, "production provenance counts differ");
  assert(counts.get("ContentIdentity") === 5798 && counts.get("ContentEvidenceBinding") === 6091 && counts.get("ConfigManifest") === 1, "production identity counts differ");
  for (const [name, expected] of Object.entries({
    Ability: 746, AbilityLevelParameter: 17606, AbilityResourceDelta: 513,
    AiGraph: 42, EnemyTemplate: 27, EnemyVariant: 27, Encounter: 6,
    ...
    Effect: 61, EffectGrantedAbility: 3, EffectModifierBinding: 18, ModifierDefinition: 1573,
    ModifierStackingGroup: 42, ModifierFilter: 151,
    CountdownDefinition: 1, LinkedUnitDefinition: 26,
    Operation: 367, Program: 156, ProgramStep: 405, RuleDefinition: 190, RuleSourceTag: 0, Selector: 178,
    StateSlot: 3, ValueExpression: 1820, LightCone: 165, LightConeStat: 14190,
  ...
  assert(identities.filter((row) => value(row, "enabled") === true).length === 5798, "production enabled identity count differs");
  const coverage = Object.groupBy(identities, (row) => value(row, "coverage_state"));
  assert((coverage.GoldenVerified?.length ?? 0) === 803 && (coverage.DataReady?.length ?? 0) === 4995, "released content coverage states differ");
