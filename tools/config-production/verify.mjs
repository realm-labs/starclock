...
function verifyGeneratedOutput(directory) {
  ...
  assert(counts.get("ContentIdentity") === 1118 && counts.get("ContentEvidenceBinding") === 1162 && counts.get("ConfigManifest") === 1, "production identity counts differ");
  for (const [name, expected] of Object.entries({
    Ability: 165, AbilityLevelParameter: 2824, AbilityResourceDelta: 67,
    AiGraph: 17, EnemyTemplate: 17, EnemyVariant: 17, Encounter: 6,
    StandardProfile: 1, StandardScenario: 6, HitPlan: 108,
    Character: 14, CharacterStat: 1204, CharacterResource: 13,
    CharacterAbilityBinding: 97, TraceNode: 260, TracePatch: 144, Eidolon: 84, EidolonPatch: 71,
    Effect: 3, EffectModifierBinding: 1, ModifierDefinition: 141,
    ModifierStackingGroup: 3, ModifierFilter: 34,
    CountdownDefinition: 1, LinkedUnitDefinition: 1,
    Operation: 27, Program: 12, RuleDefinition: 3, Selector: 19,
    StateSlot: 3, ValueExpression: 178,
  ...
  assert(identities.filter((row) => value(row, "enabled") === true).length === 879, "production enabled identity count differs");
  const coverage = Object.groupBy(identities, (row) => value(row, "coverage_state"));
  assert((coverage.GoldenVerified?.length ?? 0) === 44 && (coverage.DataReady?.length ?? 0) === 835, "character ready coverage states differ");
