...
function verifyGeneratedOutput(directory) {
  ...
  assert(counts.get("ContentIdentity") === 5278 && counts.get("ContentEvidenceBinding") === 5561 && counts.get("ConfigManifest") === 1, "production identity counts differ");
  ...
  assert(identities.filter((row) => value(row, "enabled") === true).length === 5278, "production enabled identity count differs");
  const coverage = Object.groupBy(identities, (row) => value(row, "coverage_state"));
  assert((coverage.GoldenVerified?.length ?? 0) === 283 && (coverage.DataReady?.length ?? 0) === 4995, "released content coverage states differ");
