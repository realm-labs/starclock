...
function verifyGeneratedOutput(directory) {
  ...
  assert(counts.get("ContentIdentity") === 4821 && counts.get("ContentEvidenceBinding") === 5019 && counts.get("ConfigManifest") === 1, "production identity counts differ");
  for (const [name, expected] of Object.entries({
    ...
    Effect: 4, EffectGrantedAbility: 3, EffectModifierBinding: 1, ModifierDefinition: 1196,
    ModifierStackingGroup: 18, ModifierFilter: 151,
    CountdownDefinition: 1, LinkedUnitDefinition: 1,
    Operation: 30, Program: 14, ProgramStep: 35, RuleDefinition: 83, RuleSourceTag: 0, Selector: 50,
    StateSlot: 3, ValueExpression: 1236, LightCone: 80, LightConeStat: 6880,
    LightConeSuperimposition: 1345,
  ...
  assert(identities.filter((row) => value(row, "enabled") === true).length === 4736, "production enabled identity count differs");
  const coverage = Object.groupBy(identities, (row) => value(row, "coverage_state"));
  assert((coverage.GoldenVerified?.length ?? 0) === 198 && (coverage.DataReady?.length ?? 0) === 4538, "released content coverage states differ");
