...
function verifyGeneratedOutput(directory) {
  ...
  assert(counts.get("SourceRecord") === 14 && counts.get("EvidenceRecord") === 15, "production provenance counts differ");
  assert(counts.get("ContentIdentity") === 5992 && counts.get("ContentEvidenceBinding") === 6296 && counts.get("ConfigManifest") === 1, "production identity counts differ");
  for (const [name, expected] of Object.entries({
    Ability: 797, AbilityLevelParameter: 17606, AbilityResourceDelta: 513,
    AiGraph: 57, EnemyTemplate: 38, EnemyVariant: 38, Encounter: 6,
    ...
    Effect: 80, EffectGrantedAbility: 3, EffectModifierBinding: 20, ModifierDefinition: 1575,
    ModifierStackingGroup: 44, ModifierFilter: 151,
    CountdownDefinition: 1, LinkedUnitDefinition: 35,
    Operation: 455, Program: 213, ProgramStep: 499, RuleDefinition: 191, RuleSourceTag: 0, Selector: 194,
    StateSlot: 3, ValueExpression: 1879, LightCone: 165, LightConeStat: 14190,
  ...
  assert(identities.filter((row) => value(row, "enabled") === true).length === 5992, "production enabled identity count differs");
  const coverage = Object.groupBy(identities, (row) => value(row, "coverage_state"));
  assert((coverage.GoldenVerified?.length ?? 0) === 999 && (coverage.DataReady?.length ?? 0) === 4993, "released content coverage states differ");
