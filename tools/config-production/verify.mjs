...
function verifyGeneratedOutput(directory) {
  ...
  assert(counts.get("SourceRecord") === 19 && counts.get("EvidenceRecord") === 20, "production provenance counts differ");
  assert(counts.get("ContentIdentity") === 6682 && counts.get("ContentEvidenceBinding") === 7036 && counts.get("ConfigManifest") === 1, "production identity counts differ");
  for (const [name, expected] of Object.entries({
    Ability: 997, AbilityLevelParameter: 17606, AbilityResourceDelta: 513,
    AiGraph: 117, EnemyTemplate: 88, EnemyVariant: 88, Encounter: 6,
    ...
    Effect: 141, EffectGrantedAbility: 3, EffectModifierBinding: 20, ModifierDefinition: 1575,
    ModifierStackingGroup: 44, ModifierFilter: 151,
    CountdownDefinition: 1, LinkedUnitDefinition: 43,
    Operation: 728, Program: 423, ProgramStep: 772, RuleDefinition: 196, RuleSourceTag: 0, Selector: 240,
    StateSlot: 3, ValueExpression: 2082, LightCone: 165, LightConeStat: 14190,
  ...
  assert(identities.filter((row) => value(row, "enabled") === true).length === 6682, "production enabled identity count differs");
  const coverage = Object.groupBy(identities, (row) => value(row, "coverage_state"));
  assert((coverage.GoldenVerified?.length ?? 0) === 1699 && (coverage.DataReady?.length ?? 0) === 4983, "released content coverage states differ");
