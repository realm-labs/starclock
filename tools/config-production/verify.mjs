...
function verifyGeneratedOutput(directory) {
  ...
  assert(counts.get("SourceRecord") === 3 && counts.get("EvidenceRecord") === 4, "production provenance counts differ");
  assert(counts.get("ContentIdentity") === 5346 && counts.get("ContentEvidenceBinding") === 5630 && counts.get("ConfigManifest") === 1, "production identity counts differ");
  for (const [name, expected] of Object.entries({
    Ability: 669, AbilityLevelParameter: 17606, AbilityResourceDelta: 513,
    AiGraph: 20, EnemyTemplate: 18, EnemyVariant: 18, Encounter: 6,
    ...
    Effect: 12, EffectGrantedAbility: 3, EffectModifierBinding: 4, ModifierDefinition: 1559,
    ModifierStackingGroup: 28, ModifierFilter: 151,
    CountdownDefinition: 1, LinkedUnitDefinition: 8,
    Operation: 73, Program: 29, ProgramStep: 75, RuleDefinition: 170, RuleSourceTag: 0, Selector: 72,
    StateSlot: 3, ValueExpression: 1633, LightCone: 165, LightConeStat: 14190,
  ...
  assert(identities.filter((row) => value(row, "enabled") === true).length === 5346, "production enabled identity count differs");
  const coverage = Object.groupBy(identities, (row) => value(row, "coverage_state"));
  assert((coverage.GoldenVerified?.length ?? 0) === 351 && (coverage.DataReady?.length ?? 0) === 4995, "released content coverage states differ");
