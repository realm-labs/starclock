...
function verifyGeneratedOutput(directory) {
  ...
  assert(counts.get("SourceRecord") === 11 && counts.get("EvidenceRecord") === 12, "production provenance counts differ");
  assert(counts.get("ContentIdentity") === 5752 && counts.get("ContentEvidenceBinding") === 6044 && counts.get("ConfigManifest") === 1, "production identity counts differ");
  for (const [name, expected] of Object.entries({
    Ability: 736, AbilityLevelParameter: 17606, AbilityResourceDelta: 513,
    AiGraph: 39, EnemyTemplate: 26, EnemyVariant: 26, Encounter: 6,
    ...
    Effect: 56, EffectGrantedAbility: 3, EffectModifierBinding: 17, ModifierDefinition: 1572,
    ModifierStackingGroup: 41, ModifierFilter: 151,
    CountdownDefinition: 1, LinkedUnitDefinition: 26,
    Operation: 339, Program: 144, ProgramStep: 376, RuleDefinition: 189, RuleSourceTag: 0, Selector: 167,
    StateSlot: 3, ValueExpression: 1803, LightCone: 165, LightConeStat: 14190,
  ...
  assert(identities.filter((row) => value(row, "enabled") === true).length === 5752, "production enabled identity count differs");
  const coverage = Object.groupBy(identities, (row) => value(row, "coverage_state"));
  assert((coverage.GoldenVerified?.length ?? 0) === 757 && (coverage.DataReady?.length ?? 0) === 4995, "released content coverage states differ");
