...
function verifyGeneratedOutput(directory) {
  ...
  assert(counts.get("SourceRecord") === 18 && counts.get("EvidenceRecord") === 19, "production provenance counts differ");
  assert(counts.get("ContentIdentity") === 6545 && counts.get("ContentEvidenceBinding") === 6888 && counts.get("ConfigManifest") === 1, "production identity counts differ");
  for (const [name, expected] of Object.entries({
    Ability: 958, AbilityLevelParameter: 17606, AbilityResourceDelta: 513,
    AiGraph: 105, EnemyTemplate: 77, EnemyVariant: 77, Encounter: 6,
    ...
    Effect: 129, EffectGrantedAbility: 3, EffectModifierBinding: 20, ModifierDefinition: 1575,
    ModifierStackingGroup: 44, ModifierFilter: 151,
    CountdownDefinition: 1, LinkedUnitDefinition: 43,
    Operation: 681, Program: 382, ProgramStep: 725, RuleDefinition: 195, RuleSourceTag: 0, Selector: 230,
    StateSlot: 3, ValueExpression: 2042, LightCone: 165, LightConeStat: 14190,
  ...
  assert(identities.filter((row) => value(row, "enabled") === true).length === 6545, "production enabled identity count differs");
  const coverage = Object.groupBy(identities, (row) => value(row, "coverage_state"));
  assert((coverage.GoldenVerified?.length ?? 0) === 1561 && (coverage.DataReady?.length ?? 0) === 4984, "released content coverage states differ");
