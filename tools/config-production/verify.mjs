...
function verifyGeneratedOutput(directory) {
  ...
  assert(counts.get("SourceRecord") === 13 && counts.get("EvidenceRecord") === 14, "production provenance counts differ");
  assert(counts.get("ContentIdentity") === 5865 && counts.get("ContentEvidenceBinding") === 6159 && counts.get("ConfigManifest") === 1, "production identity counts differ");
  for (const [name, expected] of Object.entries({
    Ability: 762, AbilityLevelParameter: 17606, AbilityResourceDelta: 513,
    AiGraph: 45, EnemyTemplate: 28, EnemyVariant: 28, Encounter: 6,
    ...
    Effect: 68, EffectGrantedAbility: 3, EffectModifierBinding: 20, ModifierDefinition: 1575,
    ModifierStackingGroup: 44, ModifierFilter: 151,
    CountdownDefinition: 1, LinkedUnitDefinition: 33,
    Operation: 406, Program: 176, ProgramStep: 450, RuleDefinition: 190, RuleSourceTag: 0, Selector: 186,
    StateSlot: 3, ValueExpression: 1847, LightCone: 165, LightConeStat: 14190,
  ...
  assert(identities.filter((row) => value(row, "enabled") === true).length === 5865, "production enabled identity count differs");
  const coverage = Object.groupBy(identities, (row) => value(row, "coverage_state"));
  assert((coverage.GoldenVerified?.length ?? 0) === 870 && (coverage.DataReady?.length ?? 0) === 4995, "released content coverage states differ");
