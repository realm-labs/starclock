...
function verifyGeneratedOutput(directory) {
  ...
  assert(counts.get("ContentIdentity") === 4763 && counts.get("ContentEvidenceBinding") === 4945 && counts.get("ConfigManifest") === 1, "production identity counts differ");
  for (const [name, expected] of Object.entries({
    ...
    Effect: 4, EffectGrantedAbility: 3, EffectModifierBinding: 1, ModifierDefinition: 1156,
    ModifierStackingGroup: 17, ModifierFilter: 151,
    CountdownDefinition: 1, LinkedUnitDefinition: 1,
    Operation: 30, Program: 14, ProgramStep: 35, RuleDefinition: 67, RuleSourceTag: 0, Selector: 48,
    StateSlot: 3, ValueExpression: 1196, LightCone: 64, LightConeStat: 5504,
    LightConeSuperimposition: 1050,
  ...
  assert(identities.filter((row) => value(row, "enabled") === true).length === 4662, "production enabled identity count differs");
  const coverage = Object.groupBy(identities, (row) => value(row, "coverage_state"));
  assert((coverage.GoldenVerified?.length ?? 0) === 182 && (coverage.DataReady?.length ?? 0) === 4480, "released content coverage states differ");
