...
assert(report.summary.data_ready === 30 && report.summary.golden_verified === 30 && report.summary.enabled_incomplete === 253, "runtime readiness count differs");
assert(report.summary.terminal_state_counts.Cataloged === 165, "Cataloged terminal count mismatch");
assert(report.summary.terminal_state_counts.Documented === 78, "Documented terminal count mismatch");
assert(report.summary.terminal_state_counts.Researching === 10, "Researching terminal count mismatch");
assert(report.summary.terminal_state_counts.GoldenVerified === 30, "GoldenVerified terminal count mismatch");
...
assert(report.entries.filter((entry) => entry.milestones.DataReady && entry.milestones.GoldenVerified && entry.data_ready_blockers.length === 0).length === 30, "Standard-v1 readiness differs");
assert(report.entries.filter((entry) => !entry.milestones.DataReady && !entry.milestones.GoldenVerified && entry.data_ready_blockers.length === 4).length === 253, "pending-content readiness differs");
...
assert(report.accounting.missing_data_ready_ids.length === 253, "missing DataReady ID list mismatch");
...
console.log(`Goal coverage verified (${index.coverage_sha256}; 283/283 accounted; 30 DataReady; 2 disabled audit-only).`);
