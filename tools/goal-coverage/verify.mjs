...
assert(report.summary.data_ready === 76 && report.summary.golden_verified === 76 && report.summary.enabled_incomplete === 207, "runtime readiness count differs");
assert(report.summary.terminal_state_counts.Cataloged === 165, "Cataloged terminal count mismatch");
assert(report.summary.terminal_state_counts.Documented === 42, "Documented terminal count mismatch");
assert(report.summary.terminal_state_counts.Researching === 0, "Researching terminal count mismatch");
assert(report.summary.terminal_state_counts.GoldenVerified === 76, "GoldenVerified terminal count mismatch");
...
assert(report.entries.filter((entry) => entry.milestones.DataReady && entry.milestones.GoldenVerified && entry.data_ready_blockers.length === 0).length === 76, "production readiness differs");
assert(report.entries.filter((entry) => !entry.milestones.DataReady && !entry.milestones.GoldenVerified && entry.data_ready_blockers.length === 4).length === 207, "pending-content readiness differs");
...
assert(report.accounting.missing_data_ready_ids.length === 207, "missing DataReady ID list mismatch");
...
console.log(`Goal coverage verified (${index.coverage_sha256}; 283/283 accounted; 76 DataReady; 2 disabled audit-only).`);
