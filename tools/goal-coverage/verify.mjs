...
assert(report.summary.data_ready === 283 && report.summary.golden_verified === 283 && report.summary.enabled_incomplete === 0, "runtime readiness count differs");
assert(report.summary.terminal_state_counts.Cataloged === 0, "Cataloged terminal count mismatch");
...
assert(report.summary.terminal_state_counts.GoldenVerified === 283, "GoldenVerified terminal count mismatch");
...
assert(report.entries.filter((entry) => entry.milestones.DataReady && entry.milestones.GoldenVerified && entry.data_ready_blockers.length === 0).length === 283, "production readiness differs");
assert(report.entries.filter((entry) => !entry.milestones.DataReady && !entry.milestones.GoldenVerified && entry.data_ready_blockers.length === 4).length === 0, "pending-content readiness differs");
...
assert(report.accounting.missing_data_ready_ids.length === 0, "missing DataReady ID list mismatch");
...
console.log(`Goal coverage verified (${index.coverage_sha256}; 283/283 accounted; 283 DataReady; 2 disabled audit-only).`);
