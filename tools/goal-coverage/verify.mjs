...
assert(report.summary.data_ready === 262 && report.summary.golden_verified === 262 && report.summary.enabled_incomplete === 21, "runtime readiness count differs");
assert(report.summary.terminal_state_counts.Cataloged === 21, "Cataloged terminal count mismatch");
...
assert(report.summary.terminal_state_counts.GoldenVerified === 262, "GoldenVerified terminal count mismatch");
...
assert(report.entries.filter((entry) => entry.milestones.DataReady && entry.milestones.GoldenVerified && entry.data_ready_blockers.length === 0).length === 262, "production readiness differs");
assert(report.entries.filter((entry) => !entry.milestones.DataReady && !entry.milestones.GoldenVerified && entry.data_ready_blockers.length === 4).length === 21, "pending-content readiness differs");
...
assert(report.accounting.missing_data_ready_ids.length === 21, "missing DataReady ID list mismatch");
...
console.log(`Goal coverage verified (${index.coverage_sha256}; 283/283 accounted; 262 DataReady; 2 disabled audit-only).`);
