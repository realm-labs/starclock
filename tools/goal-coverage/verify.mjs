...
assert(report.summary.data_ready === 0 && report.summary.enabled_incomplete === 283, "initial runtime readiness is overstated");
assert(report.summary.terminal_state_counts.Cataloged === 188, "Cataloged terminal count mismatch");
assert(report.summary.terminal_state_counts.Documented === 85, "Documented terminal count mismatch");
...
assert(report.entries.every((entry) => entry.milestones.DataReady === false && entry.milestones.GoldenVerified === false && entry.data_ready_blockers.length === 4), "entry readiness is overstated");
...
assert(report.accounting.missing_data_ready_ids.length === 283, "missing DataReady ID list mismatch");
...
console.log(`Goal coverage verified (${index.coverage_sha256}; 283/283 accounted; 0 DataReady; 2 disabled audit-only).`);
