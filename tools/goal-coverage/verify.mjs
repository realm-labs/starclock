...
assert(report.summary.data_ready === 36 && report.summary.golden_verified === 36 && report.summary.enabled_incomplete === 247, "runtime readiness count differs");
...
assert(report.summary.terminal_state_counts.GoldenVerified === 36, "GoldenVerified terminal count mismatch");
...
assert(report.entries.filter((entry) => entry.milestones.DataReady && entry.milestones.GoldenVerified && entry.data_ready_blockers.length === 0).length === 36, "production readiness differs");
assert(report.entries.filter((entry) => !entry.milestones.DataReady && !entry.milestones.GoldenVerified && entry.data_ready_blockers.length === 4).length === 247, "pending-content readiness differs");
...
assert(report.accounting.missing_data_ready_ids.length === 247, "missing DataReady ID list mismatch");
...
console.log(`Goal coverage verified (${index.coverage_sha256}; 283/283 accounted; 36 DataReady; 2 disabled audit-only).`);
