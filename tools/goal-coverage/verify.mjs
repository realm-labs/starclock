...
assert(report.summary.data_ready === 118 && report.summary.golden_verified === 118 && report.summary.enabled_incomplete === 165, "runtime readiness count differs");
...
assert(report.summary.terminal_state_counts.GoldenVerified === 118, "GoldenVerified terminal count mismatch");
...
assert(report.entries.filter((entry) => entry.milestones.DataReady && entry.milestones.GoldenVerified && entry.data_ready_blockers.length === 0).length === 118, "production readiness differs");
assert(report.entries.filter((entry) => !entry.milestones.DataReady && !entry.milestones.GoldenVerified && entry.data_ready_blockers.length === 4).length === 165, "pending-content readiness differs");
...
assert(report.accounting.missing_data_ready_ids.length === 165, "missing DataReady ID list mismatch");
...
console.log(`Goal coverage verified (${index.coverage_sha256}; 283/283 accounted; 118 DataReady; 2 disabled audit-only).`);
