...
assert(report.summary.data_ready === 60 && report.summary.golden_verified === 60 && report.summary.enabled_incomplete === 223, "runtime readiness count differs");
assert(report.summary.terminal_state_counts.Cataloged === 165, "Cataloged terminal count mismatch");
assert(report.summary.terminal_state_counts.Documented === 54, "Documented terminal count mismatch");
assert(report.summary.terminal_state_counts.Researching === 4, "Researching terminal count mismatch");
assert(report.summary.terminal_state_counts.GoldenVerified === 60, "GoldenVerified terminal count mismatch");
...
assert(report.entries.filter((entry) => entry.milestones.DataReady && entry.milestones.GoldenVerified && entry.data_ready_blockers.length === 0).length === 60, "production readiness differs");
assert(report.entries.filter((entry) => !entry.milestones.DataReady && !entry.milestones.GoldenVerified && entry.data_ready_blockers.length === 4).length === 223, "pending-content readiness differs");
...
assert(report.accounting.missing_data_ready_ids.length === 223, "missing DataReady ID list mismatch");
...
console.log(`Goal coverage verified (${index.coverage_sha256}; 283/283 accounted; 60 DataReady; 2 disabled audit-only).`);
