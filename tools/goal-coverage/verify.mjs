...
assert(report.summary.data_ready === 150 && report.summary.golden_verified === 150 && report.summary.enabled_incomplete === 133, "runtime readiness count differs");
assert(report.summary.terminal_state_counts.Cataloged === 133, "Cataloged terminal count mismatch");
...
assert(report.summary.terminal_state_counts.GoldenVerified === 150, "GoldenVerified terminal count mismatch");
...
assert(report.entries.filter((entry) => entry.milestones.DataReady && entry.milestones.GoldenVerified && entry.data_ready_blockers.length === 0).length === 150, "production readiness differs");
assert(report.entries.filter((entry) => !entry.milestones.DataReady && !entry.milestones.GoldenVerified && entry.data_ready_blockers.length === 4).length === 133, "pending-content readiness differs");
...
assert(report.accounting.missing_data_ready_ids.length === 133, "missing DataReady ID list mismatch");
...
console.log(`Goal coverage verified (${index.coverage_sha256}; 283/283 accounted; 150 DataReady; 2 disabled audit-only).`);
