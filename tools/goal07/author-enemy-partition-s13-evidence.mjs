...
const partitionId = "G07-P5-M15-S13";
const write = process.argv.includes("--write");
assert(
  process.argv.slice(2).every((argument) => argument === "--write"),
  "usage: author-enemy-partition-s13-evidence.mjs [--write]",
);
...
const specs = [
  {
    id: "enemy.automaton-spider.minionlv2.variant.01",
    source: "1012020",
    levels: [31, 32, 34, 37, 51, 54],
    accuracy: "ExactPublic",
    publicUrl: "https://honkai-star-rail.fandom.com/wiki/Automaton_Spider",
  },
  {
    id: "enemy.baryon.minion.variant.01",
    source: "8011010",
    levels: [7, 21, 22, 24, 25, 51],
    accuracy: "ApprovedNumericApproximation",
  },
  {
    id: "enemy.cloud-knights-patroller.minionlv2.variant.01",
    source: "2002010",
    levels: [51, 52, 54],
    accuracy: "ApprovedNumericApproximation",
  },
  {
    id: "enemy.decaying-shadow.elite.variant.01",
    source: "8003040",
    levels: [36, 49, 50, 61, 66, 75, 84],
    accuracy: "ApprovedNumericApproximation",
  },
  {
    id: "enemy.disciples-of-sanctus-medicus-ballistarius.minionlv2.variant.01",
    source: "2022060",
    levels: [54, 55, 57],
    accuracy: "ExactPublic",
    publicUrl:
      "https://honkai-star-rail.fandom.com/wiki/Disciples_of_Sanctus_Medicus%3A_Ballistarius",
  },
  {
    id: "enemy.disciples-of-sanctus-medicus-internal-alchemist.minionlv2.variant.01",
    source: "2022020",
    levels: [54, 57],
    accuracy: "ApprovedNumericApproximation",
  },
  {
    id: "enemy.disciples-of-sanctus-medicus-shape-shifter-bug.elite.variant.01",
    source: "2023011",
    levels: [53, 69, 78, 87],
    accuracy: "ApprovedNumericApproximation",
  },
  {
    id: "enemy.disciples-of-sanctus-medicus-shape-shifter.elite.variant.01",
    source: "2023010",
    levels: [44, 66, 75, 84],
    accuracy: "ExactPublic",
    publicUrl:
      "https://honkai-star-rail.fandom.com/wiki/Disciples_of_Sanctus_Medicus%3A_Shape_Shifter",
  },
  {
    id: "enemy.dreamjolt-troupes-beyond-overcooked-bug.elite.variant.01",
    source: "3003012",
    levels: [47],
    accuracy: "ApprovedNumericApproximation",
  },
  {
    id: "enemy.dreamjolt-troupes-beyond-overcooked.elite.variant.01",
    source: "3003010",
    levels: [44, 50, 66, 75, 84],
    accuracy: "ApprovedNumericApproximation",
  },
  {
    id: "enemy.dreamjolt-troupes-birdskull.minion.variant.01",
    source: "3001020",
    levels: [51, 54, 57],
    accuracy: "ApprovedNumericApproximation",
  },
  {
    id: "enemy.dreamjolt-troupes-bubble-hound.minionlv2.variant.01",
    source: "3002030",
    levels: [51, 54, 55, 57],
    accuracy: "ApprovedNumericApproximation",
  },
...
const anchor = {
  ...
  numeric_policy_id: "goal07-s13-public-and-derived-level-curve-v1",
  source: {
    publisher: "Honkai: Star Rail Wiki and committed version 4.4 extraction",
    url: "https://honkai-star-rail.fandom.com/wiki/Automaton_Spider",
  ...
  derivation: {
    ...
    exact_overrides:
      "ExactPublic variants use the reviewed public level curve; all other rows use the same named normalized template curve.",
  },
  variants: specs.map((spec) => {
    ...
    return {
      ...
      levels: spec.levels.map((level) => ({
        ...
        base_hp: String(roundHalfAway((hp930[level] * baseHp) / 930)),
...
const review = {
  ...
  sources: [
    ...
    source(variantsPath, [
      "weaknesses, resistances, debuff resistances and summon references",
    ]),
    source(difficultiesPath, [
      "all frozen Standard Universe elite difficulty levels",
    ]),
    source(encountersPath, [
      "11 frozen encounter groups, 43 members and hard level group 2",
    ]),
    {
      path: anchorPath,
      sha256: anchorDigest,
      facts: [
        "exact-public Automaton Spider, Ballistarius and Shape Shifter rows",
  ...
  reviewed_runtime_boundaries: [
    ...
    "Automaton Spider retains Timed Module, Self-Exploding Module and its killing-blow Chains of Destruction damage.",
    "Ballistarius retains six-hit random-target Mounted Crossbow.",
    "Shape Shifter retains its HP-consuming two-unit summon and Vigor Draining healing boundary.",
    "Decaying Shadow retains the Binding and Liberation sequence; Beyond Overcooked variants retain stove charge, ignition/detonation and party damage.",
    "The nine inherited approximate variants use the named normalized template curve while preserving executable source-ordered mechanics.",
...
console.log(
  `${write ? "Authored" : "Verified"} Goal 07 S13 numeric anchors and source review.`,
);
