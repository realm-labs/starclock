...
assert(options.includes("--release"), "Goal 14 verifier requires --release");
const bless = options.includes("--bless");
const requireClean = options.includes("--require-clean");
const policyPath = "policy/goal14-release-contract.json";
const evidencePath = "evidence/gold-and-gears-runtime-v1/release/release-evidence.json";
...
assert(policy.schema_revision === "starclock.goal14-release-contract.v1" &&
  policy.goal_id === "gold-and-gears-runtime-v1" && policy.state === "Released",
"Goal 14 release identity drift");
assert(policy.planned_phases === 9 && policy.planned_fixed_batches === 39 &&
  policy.planned_mechanic_partitions === 9 && policy.planned_total_batches === 48 &&
  policy.release_batch === "G14-P8-B4", "Goal 14 phase or batch denominator drift");
assert(Object.values(policy.contracts).every((value) => value === true),
  "Goal 14 release contracts must all be enabled");
for (const collection of [policy.policy_files, policy.evidence_files, policy.documentation_files]) {
  assert(Array.isArray(collection) && collection.length > 0 &&
    new Set(collection).size === collection.length, "Goal 14 release inventory is invalid");
  for (const relative of collection)
    assert(sourceExists(relative), `Goal 14 release input is missing ${relative}`);
}

if (snapshot) {
  assert(capture("git", ["cat-file", "-t", completionCommit]) === "commit",
    "Goal 14 completion commit is unavailable");
  assert(capture("git", ["show", "-s", "--format=%T", completionCommit]) === snapshot.completion_tree,
    "Goal 14 completion tree differs from its snapshot");
  assert(snapshot.status_path === "docs/goals/14-gold-and-gears-runtime-status.md" &&
    snapshot.release_policy_path === policyPath && snapshot.release_evidence_path === evidencePath &&
    snapshot.evidence_schema_revision === "starclock.goal14-release-evidence.v1",
  "Goal 14 snapshot paths drift");
}

const status = sourceText("docs/goals/14-gold-and-gears-runtime-status.md");
const batchIds = [...status.matchAll(/^\| `(G14-P\d+-(?:B|M)\d+)` \|/gmu)].map((match) => match[1]);
const completed = [...status.matchAll(
  /^\| `(G14-P\d+-(?:B|M)\d+)` \| `Complete` \|/gmu,
)].map((match) => match[1]);
assert(batchIds.length === 48 && new Set(batchIds).size === 48 && completed.length === 48,
  "Goal 14 batch ledger is incomplete");
assert(status.includes("| State | `Complete` |") && status.includes("| Active batch | None |") &&
  status.includes("| Next unblocked batch | None |") &&
  status.includes("| `G14-P8-B4` | `Complete` |") && !status.includes("- [ ]"),
"Goal 14 status or terminal checklist is incomplete");
...
"Goal 14 completeness denominator drift");
assert(completeness.source_exact_once.gaps === 0 && completeness.source_exact_once.duplicates === 0 &&
  ...
  completeness.fixture_exact_once.duplicates === 0, "Goal 14 exact-once release drift");
assert(matrix.result === "Pass" && matrix.execution.completed_runs === terminal.seeded_complete_runs &&
  ...
  matrix.execution.total_nested_battles === terminal.nested_battles &&
  matrix.execution.runtime_json_reads === 0, "Goal 14 seeded matrix drift");
assert(replay.result === "Pass" &&
  ...
  replay.representative_complete_run.real_nested_battles === 17,
"Goal 14 representative replay drift");
...
"Goal 14 controller or external-surface parity drift");
assert(hardening.result === "cross-platform-native-contract-and-local-windows-vectors-frozen" &&
...
"Goal 14 hardening denominator drift");
...
"Goal 14 performance evidence drift");
...
"Goal 14 release-audit evidence drift");

const snapshotIds = new Set(snapshots.goals.map(({ goal_id: goalId }) => goalId));
for (const goalId of policy.required_prior_contracts)
  assert(snapshotIds.has(goalId), `Goal 14 prior release snapshot is missing ${goalId}`);

const report = {
  schema_revision: "starclock.goal14-release-evidence.v1",
  goal_id: policy.goal_id,
  released_on: policy.released_on,
  result: "complete",
  completion_commit: "this file's containing G14-P8-B4 commit",
  completion: {
  ...
  production_matrix: {
    ...
    nested_battles: matrix.execution.total_nested_battles,
    difficulties: matrix.frozen_matrix.difficulties,
    paths: matrix.frozen_matrix.paths,
    custom_dice: matrix.frozen_matrix.custom_dice,
...
if (bless) {
  assert(!snapshot, "Goal 14 release evidence cannot be re-blessed after snapshot registration");
  ...
  assert(existing.replaceAll("\r\n", "\n") === output, "Goal 14 release evidence is stale");
}

if (snapshot) verifyHostedEvidence(snapshot);
if (requireClean) {
  assert(snapshot, "Goal 14 completion snapshot is not registered");
  assert(capture("git", ["status", "--porcelain"]) === "", "Goal 14 worktree is not clean");
}
console.log(`Goal 14 release verified (48 batches${snapshot ? ", snapshot and hosted native evidence" : ", pre-snapshot"}${requireClean ? ", clean" : ""}).`);

function verifyHostedEvidence(releaseSnapshot) {
  const hosted = currentJson(policy.hosted_native_evidence);
  assert(hosted.schema_revision === "starclock.goal14-hosted-native-ci.v1" &&
  ...
  "Goal 14 hosted native evidence identity drift");
  ...
  "Goal 14 hosted CI profile receipts drift");
