...
const ai = packages.find((entry) => entry.name === "starclock-ai");
assert(ai.dependencies.every((dependency) => dependency.kind === "dev" ? ["serde_json", "starclock-data"].includes(dependency.name) : ["sha2", "starclock-activity", "starclock-combat", "starclock-mode-currency-wars", "starclock-replay"].includes(dependency.name)), "starclock-ai may depend only on reviewed Activity/combat/mode/replay boundaries, the private SHA-256 backend and data-backed matrix fixtures");
...
assert(currencyWars.dependencies.every((dependency) => ["sha2", "starclock-activity", "starclock-build", "starclock-combat"].includes(dependency.name)), "starclock-mode-currency-wars may depend only on generic Activity/build/combat domains and the reviewed private SHA-256 backend");
...
assert(fateNight.dependencies.every((dependency) => ["starclock-activity", "starclock-combat"].includes(dependency.name)), "starclock-mode-fate-night may depend only on the generic Activity and combat domains");
...
assert(agentApi.dependencies.every((dependency) => ["starclock-activity", "starclock-ai", "starclock-combat", "starclock-data", "starclock-mode-currency-wars", "starclock-mode-universe", "starclock-replay", "serde", "serde_json", "sha2"].includes(dependency.name) || (dependency.kind === "dev" && ["allocation-counter", "proptest"].includes(dependency.name))), "starclock-agent-api may use only reviewed combat/Activity controller, composition and replay boundaries, deterministic serialization/token-digest dependencies and property/benchmark tooling");
