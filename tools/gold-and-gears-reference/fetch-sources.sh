...
if [[ ! -d "${turn_target}/.git" ]]; then
  git clone --filter=blob:none --no-checkout \
    https://gitlab.com/Dimbreath/turnbasedgamedata.git "${turn_target}"
fi
if [[ -n "$(git -C "${turn_target}" status --porcelain)" ]]; then
...
git -C "${turn_target}" sparse-checkout set --no-cone \
  ...
  '/Config/ConfigAI/' \
  '/Config/Gameplays/RogueDLC/'
git -C "${turn_target}" checkout --detach "${turn_revision}"

if [[ ! -d "${res_target}/.git" ]]; then
  git clone --filter=blob:none --no-checkout \
    https://github.com/Mar-7th/StarRailRes.git "${res_target}"
fi
if [[ -n "$(git -C "${res_target}" status --porcelain)" ]]; then
...
echo "Pinned Goal 08 source cache is ready at ${cache_root}"
