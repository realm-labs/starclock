...
export async function createContext(root) {
  ...
  const outputRoot = path.join(root, "content-reference/gold-and-gears-v1");
  ...
  function sourceRef(entry, evidenceQuality = "ExactStructured", extra = {}) {
    return {
      source_id: `source.goal08.${slug(entry.sourcePath)}.${slug(entry.locator)}`,
  ...
  async function policyRef(locator, note, replacementCondition) {
    const relative = "content-manifests/gold-and-gears-v1/normalized-schema.json";
    const bytes = await fs.readFile(path.join(root, relative));
    return {
      source_id: `source.goal08.project-policy.${slug(locator)}`,
      repository: "starclock",
      revision: "starclock.gold-and-gears-normalized-schema.v1",
      path: relative,
      locator,
      sha256: sha256(bytes),
      access_date: ACCESS_DATE,
